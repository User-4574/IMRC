				 ...
				 * Tcl_GetString(objv[1]) is "see". */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " see index\"",          NULL);
	return TCL_ERROR;
    }
    if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[2]), &index) != TCL_OK) {
	return TCL_ERROR;
    }
...
int
TkTextXviewCmd(textPtr, interp, argc, objv)
    TkText *textPtr;		/* Information about text widget. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings.  Someone else has already
				 * parsed this command enough to know that
				 * Tcl_GetString(objv[1]) is "xview". */
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
    ...
    type = Tk_GetScrollInfo(interp, argc, objv, &fraction, &count);
    switch (type) {
...
int
TkTextYviewCmd(textPtr, interp, argc, objv)
    TkText *textPtr;		/* Information about text widget. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings.  Someone else has already
				 * parsed this command enough to know that
				 * Tcl_GetString(objv[1]) is "yview". */
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
	...
	if ((switchLength >= 2)
		&& (LangCmpOpt("-pickplace", Tcl_GetString(objv[2]), switchLength) == 0)) {
	    pickPlace = 1;
	    if (argc != 4) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
	...
	if (!strchr(Tcl_GetString(objv[2+pickPlace]),'.') &&
	    Tcl_GetIntFromObj(interp, objv[2+pickPlace], &lineNum) == TCL_OK) {
	...
	if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[2+pickPlace]),
		&index) != TCL_OK) {
	    return TCL_ERROR;
	}
...
int
TkTextScanCmd(textPtr, interp, argc, objv)
    register TkText *textPtr;	/* Information about text widget. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings.  Someone else has already
				 * parsed this command enough to know that
				 * Tcl_GetString(objv[1]) is "scan". */
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
    TkTextIndex index;
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		...
		Tcl_GetString(objv[0]), " scan dragto x y ?gain?\"",          NULL);
	return TCL_ERROR;
    }
    ...
    if ((argc == 6) && (Tcl_GetIntFromObj(interp, objv[5], &gain) != TCL_OK))
	return TCL_ERROR;
