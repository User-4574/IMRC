...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_STRING, "-maskdata",          NULL,          NULL,
	         NULL, Tk_Offset(BitmapMaster, maskDataString),
	TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-maskfile",          NULL,          NULL,
	         NULL, Tk_Offset(BitmapMaster, maskFileString),
	TK_CONFIG_NULL_OK},
				 ...
				 * such as TK_CONFIG_ARGV_ONLY. */
{
    ...
    if ((masterPtr->maskFileString != NULL)
	    || (masterPtr->maskDataString != NULL)) {
	if (masterPtr->data == NULL) {
	    Tcl_SetResult(masterPtr->interp, "can't have mask without bitmap", TCL_STATIC);
	...
	if ((maskWidth != masterPtr->width)
	    ...
	    ckfree(masterPtr->maskData);
	    masterPtr->maskData = NULL;
	    Tcl_SetResult(masterPtr->interp, "bitmap and mask have different sizes", TCL_STATIC);
