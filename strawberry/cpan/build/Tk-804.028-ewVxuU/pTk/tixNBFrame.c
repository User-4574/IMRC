...
static Tk_ConfigSpec tabConfigSpecs[] = {
    ...
    {TK_CONFIG_PIXELS, "-wraplength",          NULL,          NULL,
...
static int		TabConfigure _ANSI_ARGS_((WidgetPtr wPtr,
			    Tab *tPtr, Tcl_Obj *CONST *objv, int argc));
 ...
 */
int
Tix_NoteBookFrameCmd(clientData, interp, argc, objv)
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    wPtr->widgetCmd = Lang_CreateWidget(interp,wPtr->tkwin, WidgetCommand, (ClientData) wPtr, WidgetCmdDeletedProc);
    ...
    Tcl_SetObjResult(interp,LangWidgetObj(interp,wPtr->tkwin));
    return TCL_OK;
}
    ...
    Tcl_Obj *CONST *objv;			/* Argument strings. */
{
    ...
    if (((c == 'a') && (strncmp(Tcl_GetString(objv[1]), "activate", length) == 0))||
	((c == 'f') && (strncmp(Tcl_GetString(objv[1]), "focus", length) == 0))){
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " ", Tcl_GetString(objv[1]), " name\"",          NULL);
	    goto error;
	}
	else {
		...
		for (tPtr=wPtr->tabHead; tPtr; tPtr=tPtr->next) {
		    if (strcmp(Tcl_GetString(objv[2]), tPtr->name) == 0) {
			if (c == 'a') {
			    wPtr->active = tPtr;
			    wPtr->focus  = tPtr;
			}
    ...
    else if ((c == 'a') && (strncmp(Tcl_GetString(objv[1]), "add", length) == 0)) {
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be ",
		Tcl_GetString(objv[0]), " add name ?options?",          NULL);
	    goto error;
	}
	else {
	    if (AddTab(wPtr, Tcl_GetString(objv[2]), objv+3, argc-3)!= TCL_OK) {
		goto error;
	    } else {
	...
	if (argc == 3) {
		...
		(char *)wPtr, Tcl_GetString(objv[2]), 0);
	} else {
	    result = Tix_ArgcError(interp, argc, objv, 2, "option");
	}
    }
	...
	if (argc == 2) {
	    ...
	    result = WidgetConfigure(interp, wPtr, argc-2, objv+2,
		    TK_CONFIG_ARGV_ONLY);
	}
    }
	...
	if (argc != 3) {
	    Tix_ArgcError(interp, argc, objv, 2, "page");
	    goto error;
	}

	/* Find the tab from the list */
	for (prev=tPtr=wPtr->tabHead; tPtr; prev=tPtr,tPtr=tPtr->next) {
	    if (strcmp(tPtr->name, Tcl_GetString(objv[2]))==0) {
		break;
	    }
	}
	if (tPtr == NULL) {
	    Tcl_AppendResult(wPtr->interp,
		"Unknown tab \"", Tcl_GetString(objv[2]), "\"",          NULL);
	    goto error;
	}
	if (tPtr == prev) {
    ...
    else if ((c == 'g') && (strncmp(Tcl_GetString(objv[1]), "geometryinfo", length) == 0)) {
	ComputeGeometry(wPtr);
	Tcl_IntResults(interp, 2, 0, wPtr->width, wPtr->height);
    }
    else if ((c == 'i') && (strncmp(Tcl_GetString(objv[1]), "identify", length) == 0)) {
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " identify x y\"",          NULL);
	    goto error;
	}
	else {
	    int x, y, left, right;
    ...
    else if ((c == 'i') && (strncmp(Tcl_GetString(objv[1]), "info", length) == 0)) {
	Tcl_ResetResult(interp);

	if (argc == 3 && strcmp(Tcl_GetString(objv[2]), "pages")==0 ) {
	    Tab * tPtr;
	...
	else if (argc == 3 && strcmp(Tcl_GetString(objv[2]), "active")==0 ) {
	    if (wPtr->active) {
		Tcl_AppendResult(interp, wPtr->active->name, NULL);
	    }
	}
	else if (argc == 3 && strcmp(Tcl_GetString(objv[2]), "focus")==0 ) {
	    if (wPtr->focus) {
		Tcl_AppendResult(interp, wPtr->focus->name, NULL);
	    }
	}
	else if (argc == 3 && strcmp(Tcl_GetString(objv[2]), "focusnext")==0 ) {
	    Tab * next;
	    if (wPtr->focus) {
		if (wPtr->focus->next) {
	...
	else if (argc == 3 && strcmp(Tcl_GetString(objv[2]), "focusprev")==0 ) {
	    Tab * prev, *tPtr;
    ...
    else if ((c == 'm') && (strncmp(Tcl_GetString(objv[1]), "move", length) == 0)) {

    }
    else if ((c == 'p') && (strncmp(Tcl_GetString(objv[1]), "pagecget", length) == 0)) {
	Tab * tPtr;

	if (argc != 4) {
	    Tix_ArgcError(interp, argc, objv, 2, "option");
	    goto error;
	}
	if ((tPtr=FindTab(interp, wPtr, Tcl_GetString(objv[2]))) == NULL) {
	    goto error;
	}
	result = Tk_ConfigureValue(interp, wPtr->tkwin, tabConfigSpecs,
	    (char *)tPtr, Tcl_GetString(objv[3]), 0);
    }
    else if ((c == 'p') && (strncmp(Tcl_GetString(objv[1]), "pageconfigure", length) == 0)) {
	Tab * tPtr;

	if (argc < 3) {
	    Tix_ArgcError(interp, argc, objv, 2,
		"page ?option value ...?");
	    goto error;
	}
	if ((tPtr=FindTab(interp, wPtr, Tcl_GetString(objv[2]))) == NULL) {
	...
	if (argc == 3) {
	    ...
	    result = TabConfigure(wPtr, tPtr, objv+3, argc-3);
	}
    }
    else {
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (wPtr->tkwin != NULL) {
	    wPtr->tkwin = NULL;
	    Lang_DeleteWidget(wPtr->interp,wPtr->widgetCmd);
    ...
    char * name;
{
    ...
    Tcl_AppendResult(interp, "Unknown tab \"", name, "\"",          NULL);
    return NULL;
}


static int TabConfigure(wPtr, tPtr, objv, argc)
    WidgetPtr wPtr;
    Tab *tPtr;
    Tcl_Obj *CONST *objv;
    int argc;
{
	...
	argc, objv, (char *)tPtr, TK_CONFIG_ARGV_ONLY) != TCL_OK) {
	return TCL_ERROR;
    }
    ...
    int argc;
{
    ...
    if (TabConfigure(wPtr, tPtr, objv, argc) != TCL_OK) {
	return TCL_ERROR;
    }
