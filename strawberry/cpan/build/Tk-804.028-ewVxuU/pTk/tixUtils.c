...
{
    char * p;
    if (Tcl_GetResult(interp)) {
	statePtr->result = tixStrDup(Tcl_GetResult(interp));
    } else {
...
int Tix_HandleSubCmds(cmdInfo, subCmdInfo, clientData, interp, argc, objv)
    Tix_CmdInfo * cmdInfo;
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
	    Tcl_GetString(objv[0]), " ", cmdInfo->info, "\".",          NULL);
	...
	if (s->name == TIX_DEFAULT_SUBCMD) {
	    if (s->checkArgvProc) {
	      if (!((*s->checkArgvProc)(clientData, interp, argc+1, objv+1))) {
		    /* Some improper objv in the arguments of the default
		     * subcommand
		     */
		    break;
		}
	    }
	    return (*s->proc)(clientData, interp, argc+1, objv+1);
	}

	if (s->namelen == TIX_DEFAULT_LEN) {
	    s->namelen = strlen(s->name);
	}
	if (s->name[0] == Tcl_GetString(objv[1])[0] && strncmp(Tcl_GetString(objv[1]),s->name,len)==0) {
	    if (argc < s->minargc) {
		error = WRONG_ARGC;
		break;
	    }
	     ...
	     */
	    return (*s->proc)(clientData, interp, argc, objv+2);
	}
    }
	 ...
	 */
	Tcl_AppendResult(interp, "wrong # args: should be \"",
	    Tcl_GetString(objv[0]), " ", Tcl_GetString(objv[1]), " ", s->info, "\"",          NULL);
    } else {
	int max;
	...
	if (max == 0) {
	    ...
	    Tcl_AppendResult(interp,
		" Must be ", subCmdInfo->name, ".",          NULL);
	} else {
	    Tcl_AppendResult(interp, " Must be ",          NULL);

	    for (i = 0, s = subCmdInfo; i < max; i++, s++) {
		if (i == max-1) {
		    Tcl_AppendResult(interp,"or ",s->name, ".",          NULL);
		} else if (i == max-2) {
		    Tcl_AppendResult(interp, s->name, " ",          NULL);
		} else {
		    Tcl_AppendResult(interp, s->name, ", ",          NULL);
		}
	    }
...
void Tix_Exit(interp, code)
    ...
    int code;
{
    if (code != 0 && interp && Tcl_GetResult(interp) != 0) {
    ...
    Tcl_CmdDeleteProc *deleteProc;
{
	...
	Tcl_CreateCommand(interp, cmdPtr->name, cmdPtr->cmdProc, clientData, deleteProc);
    ...
    char * name;		/* Textual name of the hash table. */
{
	...
	Tcl_Panic("Unknown hash table %s", name);
    }
}
#endif
				 ...
				 * stored. If NULL, no length is stored. */
{
	...
	if (obj->bytes != NULL) {
		...
		*lengthPtr = obj->length;
	    ...
	    return (obj->length) ? obj->bytes :          NULL;
	}
