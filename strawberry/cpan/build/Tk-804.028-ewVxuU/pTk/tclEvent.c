...
{
    Tcl_Interp *interp;
    CONST Tcl_Obj **objv = LangAllocVec(2);
    int code;
    BgError *errPtr;
	...
	LangSetString(objv+0, "bgerror");
	LangSetString(objv+1, assocPtr->firstBgPtr->errorMsg);

	Tcl_AllowExceptions(interp);
	Tcl_Preserve((ClientData) interp);
	code = TclGlobalInvoke(interp, 2, objv, 0);
	if (code == TCL_ERROR) {
		...
		TclGlobalInvoke(interp, 2, objv, TCL_INVOKE_HIDDEN);
...
TclInitSubsystems(argv0)
    CONST char *argv0;          /* Name of executable from Tcl_GetString(objv[0]) to main()
