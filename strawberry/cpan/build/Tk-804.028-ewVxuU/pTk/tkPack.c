...
PackAfter(interp, prevPtr, masterPtr, objc, objv)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
    Packer *prevPtr;		/* Pack windows in objv just after this
				 ...
				 * against which to pack. */
{
	...
	for (index = 0 ; index < optionCount; index++) {
	    ...
	    if ((c == 't') && (strncmp(curOpt, "top", (size_t) length)) == 0) {

		packPtr->side = TOP;
	    } else if ((c == 'b') && (strncmp(curOpt, "bottom", (size_t) length)) == 0) {

		packPtr->side = BOTTOM;
	    } else if ((c == 'l') && (strncmp(curOpt, "left", (size_t) length)) == 0) {

		packPtr->side = LEFT;
	    } else if ((c == 'r') && (strncmp(curOpt, "right", (size_t) length)) == 0) {

		packPtr->side = RIGHT;
	    } else if ((c == 'e') && (strncmp(curOpt, "expand", (size_t) length)) == 0) {
