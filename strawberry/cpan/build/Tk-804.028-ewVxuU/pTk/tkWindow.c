...
#endif
    {         NULL,	(int (*) _ANSI_ARGS_((ClientData, Tcl_Interp *, int, Tcl_Obj *CONST *))) NULL, NULL, 0}
};
				 ...
				 * form "prog instance". */
{
    ...
    static Tcl_Obj *CONST *objv = {         NULL};
	...
	if (winPtr->mainPtr->refCount == 0) {
	     ...
	     */
            ...
            if ((winPtr->mainPtr->interp != NULL) &&
                ...
                for (cmdPtr = commands; cmdPtr->name != NULL; cmdPtr++) {
                    Tcl_CreateCommand(winPtr->mainPtr->interp, cmdPtr->name, TkDeadAppCmd, (ClientData) NULL, (void (*) _ANSI_ARGS_((ClientData))) NULL);


                }
                Tcl_CreateCommand(winPtr->mainPtr->interp, "send", TkDeadAppCmd, (ClientData) NULL, (void (*) _ANSI_ARGS_((ClientData))) NULL);
    ...
    Tcl_Interp *interp;		/* Interpreter to initialize. */
{
	...
	/*
	 * Get the clearance to start Tk and the "objv" parameters
	 * from the master.
	 */
	Tcl_DString ds;
	...
	Tcl_DStringFree(&ds);
	/*
	 * Use the master's result as objv.
	...
	/*
	 * If there is an "objv" variable, get its value, extract out
	 * relevant arguments from it, and rewrite the variable without
	 * the arguments that we used.
	 */

	argString = Tcl_GetVar2(interp, "objv",          NULL, TCL_GLOBAL_ONLY);
    }
    objv = NULL;
    if (argString != NULL) {
	char buffer[TCL_INTEGER_SPACE];

	if (Tcl_SplitList(interp, argString, &argc, &objv) != TCL_OK) {
	    argError:
	    Tcl_AddErrorInfo(interp,
		    "\n    (processing arguments in objv variable)");
	    Tcl_MutexUnlock(&windowMutex);
	    return TCL_ERROR;
	}
	if (Tk_ParseArgv(interp, (Tk_Window) NULL, &argc, objv,
		argTable, TK_ARGV_DONT_SKIP_FIRST_ARG|TK_ARGV_NO_DEFAULTS)
		!= TCL_OK) {
	    ckfree((char *) objv);
	    goto argError;
	}
	p = Tcl_Merge(argc, objv);
	Tcl_SetVar2(interp, "objv",          NULL, p, TCL_GLOBAL_ONLY);
