...
static Tk_ConfigSpec tagConfigSpecs[] = {
    ...
    {TK_CONFIG_LANGARG, "-elide",          NULL,          NULL,
    ...
    {TK_CONFIG_FONT, "-font",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, tkfont), TK_CONFIG_NULL_OK},
	...
	TK_CONFIG_NULL_OK},
    {TK_CONFIG_CUSTOM, "-wrap",          NULL,          NULL,
				 ...
				 * Tcl_GetString(objv[1]) is "tag". */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " tag option ?arg arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    c = Tcl_GetString(objv[2])[0];
    length = strlen(Tcl_GetString(objv[2]));
    if ((c == 'a') && (strncmp(Tcl_GetString(objv[2]), "add", length) == 0)) {
	fullOption = "add";
	addTag = 1;
	...
	if (argc < 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag ", fullOption,
	...
	for (i = 4; i < argc; i += 2) {
	    ...
	    if (argc > (i+1)) {
		if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[i+1]), &index2)
			!= TCL_OK) {
		    return TCL_ERROR;
		}
    ...
    } else if ((c == 'b') && (strncmp(Tcl_GetString(objv[2]), "bind", length) == 0)) {
	if ((argc < 4) || (argc > 6)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag bind tagName ?sequence? ?command?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = TkTextCreateTag(textPtr, Tcl_GetString(objv[3]));
	...
	if (argc == 6) {
		...
		objv[5] = Tcl_NewStringObj(Tcl_GetString(objv[5])+1,-1);
		append = 1;
	    }
#endif
	    mask = Tk_CreateBinding(interp, textPtr->bindingTable,
		    (ClientData) tagPtr, Tcl_GetString(objv[4]), objv[5], append);
	    if (mask == 0) {
		return TCL_ERROR;
	    }
    ...
    } else if ((c == 'c') && (strncmp(Tcl_GetString(objv[2]), "cget", length) == 0)
	    && (length >= 2)) {
	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag cget tagName option\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag(interp, textPtr, Tcl_GetString(objv[3]));
	if (tagPtr == NULL) {
	    return TCL_ERROR;
	}
	return Tk_ConfigureValue(interp, textPtr->tkwin, tagConfigSpecs,
		(char *) tagPtr, Tcl_GetString(objv[4]), 0);
    } else if ((c == 'c') && (strncmp(Tcl_GetString(objv[2]), "configure", length) == 0)
	    && (length >= 2)) {
	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag configure tagName ?option? ?value? ",
		    "?option value ...?\"",          NULL);
	    return TCL_ERROR;
	}
	tagPtr = TkTextCreateTag(textPtr, Tcl_GetString(objv[3]));
    ...
    } else if ((c == 'd') && (strncmp(Tcl_GetString(objv[2]), "delete", length) == 0)) {
	Tcl_HashEntry *hPtr;

	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag delete tagName tagName ...\"",
		             NULL);
	    return TCL_ERROR;
	}
	for (i = 3; i < argc; i++) {
    ...
    } else if ((c == 'l') && (strncmp(Tcl_GetString(objv[2]), "lower", length) == 0)) {
	TkTextTag *tagPtr2;
	int prio;

	if ((argc != 4) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag lower tagName ?belowThis?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag(interp, textPtr, Tcl_GetString(objv[3]));
	...
	if (argc == 5) {
	    tagPtr2 = FindTag(interp, textPtr, Tcl_GetString(objv[4]));
	    if (tagPtr2 == NULL) {
		return TCL_ERROR;
	    }
	...
	TkTextRedrawTag(textPtr, (TkTextIndex *) NULL, (TkTextIndex *) NULL,
		tagPtr, 1);
    } else if ((c == 'n') && (strncmp(Tcl_GetString(objv[2]), "names", length) == 0)
	    && (length >= 2)) {
	TkTextTag **arrayPtr;
	int arraySize;
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag names ?index?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    ...
	    if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[3]), &index1)
		    != TCL_OK) {
		return TCL_ERROR;
	    }
    ...
    } else if ((c == 'n') && (strncmp(Tcl_GetString(objv[2]), "nextrange", length) == 0)
	    && (length >= 2)) {
	...
	if ((argc != 5) && (argc != 6)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag nextrange tagName index1 ?index2?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag((Tcl_Interp *) NULL, textPtr, Tcl_GetString(objv[3]));
	if (tagPtr == NULL) {
	    return TCL_OK;
	}
	if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[4]), &index1) != TCL_OK) {
	    return TCL_ERROR;
	}
	TkTextMakeByteIndex(textPtr->tree, TkBTreeNumLines(textPtr->tree),
		0, &last);
	if (argc == 5) {
	    index2 = last;
	} else if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[5]), &index2)
		!= TCL_OK) {
	    return TCL_ERROR;
	}
	...
	TkTextPrintIndex(&tSearch.curIndex, position);
	Tcl_AppendElement(interp, position);
    } else if ((c == 'p') && (strncmp(Tcl_GetString(objv[2]), "prevrange", length) == 0)
	    && (length >= 2)) {
	TkTextSearch tSearch;
	...
	if ((argc != 5) && (argc != 6)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag prevrange tagName index1 ?index2?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag((Tcl_Interp *) NULL, textPtr, Tcl_GetString(objv[3]));
    ...
    } else if ((c == 'r') && (strncmp(Tcl_GetString(objv[2]), "raise", length) == 0)
	    && (length >= 3)) {
	TkTextTag *tagPtr2;
	int prio;
	...
	if ((argc != 4) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag raise tagName ?aboveThis?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag(interp, textPtr, Tcl_GetString(objv[3]));
	...
	TkTextRedrawTag(textPtr, (TkTextIndex *) NULL, (TkTextIndex *) NULL,
		tagPtr, 1);
    } else if ((c == 'r') && (strncmp(Tcl_GetString(objv[2]), "ranges", length) == 0)
	    && (length >= 3)) {
	TkTextSearch tSearch;
	char position[TK_POS_CHARS];
	...
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " tag ranges tagName\"",          NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag((Tcl_Interp *) NULL, textPtr, Tcl_GetString(objv[3]));
	if (tagPtr == NULL) {
    ...
    } else if ((c == 'r') && (strncmp(Tcl_GetString(objv[2]), "remove", length) == 0)
	    && (length >= 2)) {
	fullOption = "remove";
	addTag = 0;
	goto addAndRemove;
    } else {
	Tcl_AppendResult(interp, "bad tag option \"", Tcl_GetString(objv[2]),
