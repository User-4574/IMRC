...
static Tk_ConfigSpec windowItemConfigSpecs[] = {
    ...
    {TK_CONFIG_SYNONYM, "-widget", "window",          NULL,
                NULL, 0, 0},
...
static int Tix_WindowItemConfigure(iPtr, argc, objv, flags)
    Tix_DItem * iPtr;
    int argc;
    Tcl_Obj *CONST *objv;
    int flags;
{
	...
	windowItemConfigSpecs,
	argc, objv, (char *)itPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
	...
	if (itPtr->tkwin != NULL) {
		...
		Tcl_AppendResult(itPtr->ddPtr->interp, "can't use ",
		    ...
		    " of ", Tk_PathName(itPtr->ddPtr->tkwin),          NULL);
		goto badWindow;
	    }
...
static int
Tix_WindowStyleConfigure(style, argc, objv, flags)
    Tix_DItemStyle *style;
    int argc;
    Tcl_Obj *CONST *objv;
    int flags;
{
	...
	if (Tk_ConfigureWidget(stylePtr->interp, stylePtr->tkwin,
	    windowStyleConfigSpecs,
	    argc, objv, (char *)stylePtr, flags) != TCL_OK) {
	    return TCL_ERROR;
	}
    }
