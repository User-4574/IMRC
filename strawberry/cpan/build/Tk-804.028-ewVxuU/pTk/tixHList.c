...
static int		WidgetCommand _ANSI_ARGS_((ClientData clientData,
			    Tcl_Interp *, int argc, Tcl_Obj *CONST *objv));
...
static int		ConfigElement _ANSI_ARGS_((WidgetPtr wPtr,
			    HListElement *chPtr, int argc, Tcl_Obj *CONST *objv,
 ...
 */
int
Tix_HListCmd(clientData, interp, argc, objv)
    ClientData clientData;
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
     ...
     */
    tkwin = Tk_CreateWindowFromPath(interp, mainw, Tcl_GetString(objv[1]),          NULL);
    if (tkwin == NULL) {
	return TCL_ERROR;
    }
    ...
    if (WidgetConfigure(interp, wPtr, argc-2, objv+2, 0) != TCL_OK) {
    ...
    Tcl_SetObjResult(interp,LangWidgetObj(interp,wPtr->dispData.tkwin));
    return TCL_OK;
}
...
static int
WidgetCommand(clientData, interp, argc, objv)
 ...
 */
static int
Tix_HLAdd(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    HListElement * chPtr;
    char * pathName = Tcl_GetString(objv[0]);
    int code = TCL_ERROR;
    ...
    if ((chPtr = NewElement(interp, wPtr, argc, objv, pathName,
 ...
 */
static int
Tix_HLAddChild(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    parentName = Tcl_GetString(objv[0]);
    if (Tcl_GetString(objv[0]) && strcmp(Tcl_GetString(objv[0]), "") == 0) {
	parentName = NULL;
    }

    argc --;
    objv ++;
    if ((chPtr = NewElement(interp, wPtr, argc, objv, NULL,
 ...
 */
static int
Tix_HLSetSite(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (strncmp(Tcl_GetString(objv[-1]), "anchor", len)==0) {
	changePtr = &wPtr->anchor;
    }
    else if (strncmp(Tcl_GetString(objv[-1]), "dragsite", len)==0) {
    ...
    len = strlen(Tcl_GetString(objv[0]));
    if (strncmp(Tcl_GetString(objv[0]), "set", len)==0) {
	if (argc == 2) {
	    if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]))) == NULL) {
		return TCL_ERROR;
	    }
	    if (*changePtr != chPtr) {
		*changePtr = chPtr;
		...
		Tk_PathName(wPtr->dispData.tkwin), " ", Tcl_GetString(objv[-1]),
		" set entryPath", NULL);
	    return TCL_ERROR;
	}
    }
    else if (strncmp(Tcl_GetString(objv[0]), "clear", len)==0) {
	if (*changePtr != NULL) {
	    *changePtr = NULL;
	    changed = 1;
	}
	...
	Tcl_AppendResult(interp, "wrong option \"", Tcl_GetString(objv[0]), "\", ",
 ...
 */
static int
Tix_HLCGet(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    return Tk_ConfigureValue(interp, wPtr->dispData.tkwin, configSpecs,
		(char *)wPtr, Tcl_GetString(objv[0]), 0);
}
 ...
 */
static int
Tix_HLConfig(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	return WidgetConfigure(interp, wPtr, argc, objv,
	    TK_CONFIG_ARGV_ONLY);
    }
}
 ...
 */
static int
Tix_HLDelete(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (strcmp(Tcl_GetString(objv[0]), "all") == 0) {
    ...
    if (argc != 2) {
	if ((strncmp(Tcl_GetString(objv[0]), "entry", len) == 0) ||
	    (strncmp(Tcl_GetString(objv[0]), "offsprings", len) == 0) ||
	    (strncmp(Tcl_GetString(objv[0]), "siblings", len) == 0)) {
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "offsprings", len) == 0) {
	Tix_HLMarkElementDirty(wPtr, chPtr);
	DeleteOffsprings(wPtr, chPtr);
    }
    else if (strncmp(Tcl_GetString(objv[0]), "siblings", len) == 0) {
	Tix_HLMarkElementDirty(wPtr, chPtr);
...
wrong_arg:
	...
	"wrong # of arguments, should be pathName delete ", Tcl_GetString(objv[0]),
	" entryPath", NULL);
    return TCL_ERROR;

wrong_option:

    Tcl_AppendResult(interp, "unknown option \"", Tcl_GetString(objv[0]),
 ...
 */
static int
Tix_HLEntryCget(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[0]))) == NULL) {
	return TCL_ERROR;
    }
    if (chPtr->col[0].iPtr == NULL) {
	Tcl_AppendResult(interp, "Item \"", Tcl_GetString(objv[0]),
	    "\" does not exist",          NULL);
	return TCL_ERROR;
    }
    return Tix_ConfigureValue2(interp, wPtr->dispData.tkwin, (char *)chPtr,
	entryConfigSpecs, chPtr->col[0].iPtr, Tcl_GetString(objv[1]), 0);
}
 ...
 */
static int
Tix_HLEntryConfig(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	return Tix_ConfigureInfo2(interp, wPtr->dispData.tkwin,
	    (char*)chPtr, entryConfigSpecs, chPtr->col[0].iPtr,
	    (char *) Tcl_GetString(objv[1]), 0);
    } else {
	return ConfigElement(wPtr, chPtr, argc-1, objv+1,
 ...
 */
static int
Tix_HLGeometryInfo(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (argc == 2) {
	if (Tcl_GetIntFromObj(interp, objv[0], &qSize[0]) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (Tcl_GetIntFromObj(interp, objv[1], &qSize[1]) != TCL_OK) {
	    return TCL_ERROR;
	}
    } else {
 ...
 */
static int
Tix_HLHide(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_HLShow(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_HLInfo(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    WidgetPtr wPtr = (WidgetPtr) clientData;
    HListElement * chPtr;
    size_t len = strlen(Tcl_GetString(objv[0]));

    if (strncmp(Tcl_GetString(objv[0]), "anchor", len)==0) {
	if (wPtr->anchor) {
	    Tcl_AppendResult(interp, wPtr->anchor->pathName, NULL);
	}
	return TCL_OK;
    }
    else if (strncmp(Tcl_GetString(objv[0]), "bbox", len)==0) {
	HListElement * chPtr;

	if (argc != 2) {
	    return Tix_ArgcError(interp, argc+2, objv-2, 3, "entryPath");
	}
	if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]))) == NULL) {
	    return TCL_ERROR;
	}
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "children", len)==0) {
	HListElement * ptr;

	if (argc != 1 && argc != 2) {
	    return Tix_ArgcError(interp, argc+2, objv-2, 3, "?entryPath?");
	}
	if (argc == 1 || (argc == 2 && Tcl_GetString(objv[1])[0]==0)) {
	    chPtr = wPtr->root;
	} else {
	    if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]))) == NULL) {
		return TCL_ERROR;
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "data", len)==0) {
	if (argc != 2) {
	    return Tix_ArgcError(interp, argc+2, objv-2, 3, "entryPath");
	}
	if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]))) == NULL) {
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "dragsite", len)==0) {
	if (wPtr->dragSite) {
	    Tcl_AppendResult(interp, wPtr->dragSite->pathName, NULL);
	}
	return TCL_OK;
    }
    else if (strncmp(Tcl_GetString(objv[0]), "dropsite", len)==0) {
	if (wPtr->dropSite) {
	    Tcl_AppendResult(interp, wPtr->dropSite->pathName, NULL);
	}
	return TCL_OK;
    }
    else if (strncmp(Tcl_GetString(objv[0]), "exists", len)==0) {
	if (argc != 2) {
	    return Tix_ArgcError(interp, argc+2, objv-2, 3, "entryPath");
	}
	chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]));
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "hidden", len)==0) {
	if (argc != 2) {
	    return Tix_ArgcError(interp, argc+2, objv-2, 3, "entryPath");
	}
	if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]))) == NULL) {
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "item", len)==0) {
	return Tix_HLItemInfo(interp, wPtr, argc-1, objv+1);
    }
    else if (strncmp(Tcl_GetString(objv[0]), "next", len)==0) {
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "parent", len)==0) {
	if (argc != 2) {
	    return Tix_ArgcError(interp, argc+2, objv-2, 3, "entryPath");
	}
	if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]))) == NULL) {
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "prev", len)==0) {
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "selection", len)==0) {
 ...
 */
int
Tix_HLItemInfo(interp, wPtr, argc, objv)
    Tcl_Interp *interp;		/* Current interpreter. */
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (argc != 2) {
	return Tix_ArgcError(interp, argc+3, objv-3, 3, "x y");
    }
    if (Tcl_GetIntFromObj(interp, objv[0], &widX) != TCL_OK) {
	return TCL_ERROR;
    }
    if (Tcl_GetIntFromObj(interp, objv[1], &widY) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 */
static int
Tix_HLNearest(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tcl_GetIntFromObj(interp, objv[0], &y) != TCL_OK) {
	return TCL_ERROR;
    }
    if (wPtr->root->dirty || wPtr->allDirty) {
	/*
 ...
 */
static int
Tix_HLSee(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	wPtr->elmToSee = tixStrDup(Tcl_GetString(objv[0]));
	return TCL_OK;
    } else {
 ...
 */
static int
Tix_HLSelection(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (strncmp(Tcl_GetString(objv[0]), "clear", len)==0) {
	if (argc == 1) {
	    ...
	    from = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]));
	    if (from == NULL) {
		code = TCL_ERROR;
		goto done;
	    }
	    ...
	    if (argc == 3) {
		to = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[2]));
		if (to == NULL) {
		    code = TCL_ERROR;
		    goto done;
		}
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "includes", len)==0) {
	if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[1]))) == NULL) {
	    code = TCL_ERROR;
	    goto done;
	}
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "get", len)==0) {
	if (argc != 1) {
	    Tix_ArgcError(interp, argc+2, objv-2, 3, "");
	    code = TCL_ERROR;
	} else {
	    code = CurSelection(interp, wPtr, wPtr->root);
	}
    }
    else if (strncmp(Tcl_GetString(objv[0]), "set", len)==0) {
	HListElement * from, * to;

	if (argc < 2 || argc > 3) {
	    Tix_ArgcError(interp, argc+2, objv-2, 3, "from ?to?");
	    code = TCL_ERROR;
	    goto done;
	}
 ...
 */
static int
Tix_HLXView(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    else if ((chPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[0]))) != NULL) {
	leftPixel = Tix_HLElementLeftOffset(wPtr, chPtr);
    }
    else if (Tcl_GetIntFromObj(interp, objv[0], &leftPixel) == TCL_OK) {
	...
	/* Tk_GetScrollInfo () wants strange argc,objv combinations .. */
	type = Tk_GetScrollInfo(interp, argc+2, objv-2, &fraction, &count);
	switch (type) {
 ...
 */
static int
Tix_HLYView(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    else if (Tcl_GetIntFromObj(interp, objv[0], &topPixel) == TCL_OK) {
	/* %% todo backward-compatible mode */
    }
    else {
	int type, count;
 ...
 */
static int
WidgetConfigure(interp, wPtr, argc, objv, flags)
					 ...
					 * Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, wPtr->dispData.tkwin, configSpecs,
	    argc, objv, (char *) wPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 */
static HListElement *
NewElement(interp, wPtr, argc, objv, pathName, defParentName, newArgc, newArgv)
    Tcl_Interp *interp;
    WidgetPtr wPtr;
    int argc;
    Tcl_Obj *CONST *objv;
...
#define FIXED_SPACE 20
	...
	size_t len;
	if (argc %2 != 0) {
	    Tcl_AppendResult(interp, "value for \"", Tcl_GetString(objv[argc-1]),
		"\" missing", NULL);
	    chPtr = NULL;
	    goto done;
	}
	for (n=i=0; i<argc; i+=2) {
	    len = strlen(Tcl_GetString(objv[i]));
	    if (strncmp(Tcl_GetString(objv[i]), "-itemtype", len) == 0) {
		ditemType = Tcl_GetString(objv[i+1]);
		goto copy;
	    }
	    else if (strncmp(Tcl_GetString(objv[i]), "-after", len) == 0) {
		afterPtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[i+1]));
		if (afterPtr == NULL) {
		    chPtr = NULL;
		    goto done;
		}
	    ...
	    else if (strncmp(Tcl_GetString(objv[i]), "-before", len) == 0) {
		beforePtr = Tix_HLFindElement(interp, wPtr, Tcl_GetString(objv[i+1]));
		if (beforePtr == NULL) {
		    chPtr = NULL;
		    goto done;
		}
	    ...
	    else if (strncmp(Tcl_GetString(objv[i]), "-at", len) == 0) {
...
static int
ConfigElement(wPtr, chPtr, argc, objv, flags, forced)
    WidgetPtr wPtr;
    HListElement *chPtr;
    int argc;
    Tcl_Obj *CONST *objv;
				 ...
				 * the DItem is initialized properly */
{
	...
	(char*)chPtr, entryConfigSpecs, chPtr->col[0].iPtr, argc, objv, flags,
