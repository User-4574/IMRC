...
static Tcl_Obj *	WrapModePrintProc _ANSI_ARGS_((ClientData clientData,
...
WrapModeParseProc(clientData, interp, tkwin, ovalue, widgRec, offset)
    ...
    Tcl_Obj *ovalue;			/* Value of option (list of tag
					 * names). */
...
static int		ConfigureText _ANSI_ARGS_((Tcl_Interp *interp,
			    TkText *textPtr, int argc, Tcl_Obj *CONST *objv,
			    int flags));
...
static int		DumpSegment _ANSI_ARGS_((Tcl_Interp *interp, char *key,
			    char *value, Tcl_Obj *arg, LangCallback *command,
...
int
Tk_TextCmd(clientData, interp, argc, objv)
    ...
    Tcl_Obj *CONST objv[];	/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " pathName ?options?\"",          NULL);
	return TCL_ERROR;
    }
    ...
    new = Tk_CreateWindowFromPath(interp, tkwin, Tcl_GetString(objv[1]),          NULL);
    if (new == NULL) {
	return TCL_ERROR;
    }
    ...
    if (ConfigureText(interp, textPtr, argc-2, objv+2, 0) != TCL_OK) {
	Tk_DestroyWindow(textPtr->tkwin);
	return TCL_ERROR;
    }
    Tcl_SetObjResult(interp,LangWidgetObj(interp,textPtr->tkwin));
...
static int
TextWidgetCmd(clientData, interp, argc, objv)
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if ((c == 'b') && (strncmp(Tcl_GetString(objv[1]), "bbox", length) == 0)) {
	int x, y, width, height;

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " bbox index\"",          NULL);
	    result = TCL_ERROR;
	    goto done;
	}
	if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[2]), &index1) != TCL_OK) {
	    result = TCL_ERROR;
	    goto done;
	}
    ...
    } else if ((c == 'c') && (strncmp(Tcl_GetString(objv[1]), "cget", length) == 0)
	    && (length >= 2)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
	...
	result = Tk_ConfigureValue(interp, textPtr->tkwin, configSpecs,
		(char *) textPtr, Tcl_GetString(objv[2]), 0);
    } else if ((c == 'c') && (strncmp(Tcl_GetString(objv[1]), "compare", length) == 0)
	    && (length >= 3)) {
	int relation, value;
	CONST char *p;
	...
	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " compare index1 op index2\"",          NULL);
	    result = TCL_ERROR;
	    goto done;
	}
	if ((TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[2]), &index1) != TCL_OK)
		|| (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[4]), &index2)
		!= TCL_OK)) {
	    result = TCL_ERROR;
	    goto done;
	}
	...
	if (p[0] == '<') {
		...
		Tcl_AppendResult(interp, "bad comparison operator \"",
			Tcl_GetString(objv[3]), "\": must be <, <=, ==, >=, >, or !=",
			         NULL);
		result = TCL_ERROR;
		goto done;
	    }
	...
	if (argc == 2) {
	    ...
	    result = ConfigureText(interp, textPtr, argc-2, objv+2,
		    TK_CONFIG_ARGV_ONLY);
	}
    } else if ((c == 'd') && (strncmp(Tcl_GetString(objv[1]), "debug", length) == 0)
	    && (length >= 3)) {
	if (argc > 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " debug boolean\"",          NULL);
	    result = TCL_ERROR;
	    goto done;
	}
	if (argc == 2) {
    ...
    } else if ((c == 'd') && (strncmp(Tcl_GetString(objv[1]), "delete", length) == 0)
	    && (length >= 3)) {
	int i;

	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " delete index1 ?index2 ...?\"",          NULL);
	    result = TCL_ERROR;
	    goto done;
	}
	if (textPtr->state == TK_STATE_NORMAL) {
		 ...
		 */
		result = DeleteChars(textPtr, Tcl_GetString(objv[2]),
			(argc == 4) ? Tcl_GetString(objv[3]) : NULL, NULL, NULL);
	    } else {
		/*
		 ...
		 */
		for (i = 0; i < argc; i++) {
		    if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[i]),
			    &indices[i]) != TCL_OK) {
			result = TCL_ERROR;
			ckfree((char *) indices);
			goto done;
    ...
    } else if ((c == 'd') && (strncmp(Tcl_GetString(objv[1]), "dlineinfo", length) == 0)
	    && (length >= 2)) {
	int x, y, width, height, base;

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " dlineinfo index\"",          NULL);
	    result = TCL_ERROR;
	    goto done;
	}
	if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[2]), &index1) != TCL_OK) {
    ...
    } else if ((c == 'e') && (strncmp(Tcl_GetString(objv[1]), "edit", length) == 0)) {
        result = TextEditCmd(textPtr, interp, argc, objv);
    } else if ((c == 'g') && (strncmp(Tcl_GetString(objv[1]), "get", length) == 0)) {
	Tcl_Obj *objPtr = NULL;
	Tcl_DString ds;
	int i, found = 0;
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " get index1 ?index2 ...?\"",          NULL);
	    result = TCL_ERROR;
	    goto done;
	}
	for (i = 2; i < argc; i += 2) {
	    if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[i]), &index1) != TCL_OK) {
		result = TCL_ERROR;
		goto done;
	    }
	    if (i+1 == argc) {
	    ...
	    } else if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[i+1]), &index2)
		    != TCL_OK) {
		if (objPtr) {
	    ...
	    /* Using <= gives perl/Tk null strings for equal case and undef
	       for reversed range - which seems "correct"
	     */
    ...
    } else if ((c == 'i') && (strncmp(Tcl_GetString(objv[1]), "index", length) == 0)
	    && (length >= 3)) {
	char buf[200];

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " index index\"",
		             NULL);
	    result = TCL_ERROR;
	    goto done;
	}
	...
	Tcl_SetResult(interp, buf, TCL_VOLATILE);
    } else if ((c == 'i') && (strncmp(Tcl_GetString(objv[1]), "insert", length) == 0)
	    && (length >= 3)) {
	int i, j, numTags;
	Tcl_Obj * *tagNames;
	...
	if (textPtr->state == TK_STATE_NORMAL) {
	    for (j = 3;  j < argc; j += 2) {
		InsertChars(textPtr, &index1, Tcl_GetString(objv[j]));
		if (argc > (j+1)) {
		    TkTextIndexForwBytes(&index1, (int) strlen(Tcl_GetString(objv[j])),
			    &index2);
			...
			TkBTreeTag(&index1, &index2,
				TkTextCreateTag(textPtr, Tcl_GetString(tagNames[i])), 1);
		    }
		    index1 = index2;
		}
	    }
	}
    } else if ((c == 'd') && (strncmp(Tcl_GetString(objv[1]), "dump", length) == 0)) {
	result = TextDumpCmd(textPtr, interp, argc, objv);
    } else if ((c == 'i') && (strncmp(Tcl_GetString(objv[1]), "image", length) == 0)) {
	result = TkTextImageCmd(textPtr, interp, argc, objv);
    } else if ((c == 'm') && (strncmp(Tcl_GetString(objv[1]), "mark", length) == 0)) {
	result = TkTextMarkCmd(textPtr, interp, argc, objv);
    } else if ((c == 's') && (strcmp(Tcl_GetString(objv[1]), "scan") == 0) && (length >= 2)) {
	result = TkTextScanCmd(textPtr, interp, argc, objv);
    } else if ((c == 's') && (strcmp(Tcl_GetString(objv[1]), "search") == 0)
	    && (length >= 3)) {
	result = TextSearchCmd(textPtr, interp, argc, objv);
    } else if ((c == 's') && (strcmp(Tcl_GetString(objv[1]), "see") == 0) && (length >= 3)) {
	result = TkTextSeeCmd(textPtr, interp, argc, objv);
    } else if ((c == 't') && (strcmp(Tcl_GetString(objv[1]), "tag") == 0)) {
	result = TkTextTagCmd(textPtr, interp, argc, objv);
    } else if ((c == 'w') && (strncmp(Tcl_GetString(objv[1]), "window", length) == 0)) {
	result = TkTextWindowCmd(textPtr, interp, argc, objv);
    } else if ((c == 'x') && (strncmp(Tcl_GetString(objv[1]), "xview", length) == 0)) {
	result = TkTextXviewCmd(textPtr, interp, argc, objv);
    } else if ((c == 'y') && (strncmp(Tcl_GetString(objv[1]), "yview", length) == 0)
	    && (length >= 2)) {
	result = TkTextYviewCmd(textPtr, interp, argc, objv);
    } else {
...
static int
ConfigureText(interp, textPtr, argc, objv, flags)
    Tcl_Interp *interp;		/* Used for error reporting. */
    register TkText *textPtr;	/* Information about widget;  may or may
				 * not already have values for some fields. */
    int argc;			/* Number of valid entries in objv. */
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, textPtr->tkwin, configSpecs,
	    argc, objv, (char *) textPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
...
static int
TextSearchCmd(textPtr, interp, argc, objv)
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    int patLength;
    CONST char *arg, *pattern, *p, *startOfLine;
    Tcl_Obj *varName;
    char buffer[20];
	...
	if (length < 2) {
	    ...
	    Tcl_AppendResult(interp, "bad switch \"", arg,
		    ...
		    "-forward, -nocase, or -regexp",          NULL);
	    return TCL_ERROR;
	}
	c = arg[1];
	if ((c == 'b') &&  LangCmpOpt("-backwards",Tcl_GetString(objv[i]),length) == 0 ) {
	    backwards = 1;
	} else if ((c == 'c') &&  LangCmpOpt("-count",Tcl_GetString(objv[i]),length) == 0 ) {
	    if (i >= (argc-1)) {
		...
		&& (strncmp(Tcl_GetString(objv[i]), "-exact", length) == 0)) {
	    exact = 1;
	} else if ((c == 'e') && (length > 2)
		&& (strncmp(Tcl_GetString(objv[i]), "-elide", length) == 0)) {
	    searchElide = 1;
	} else if ((c == 'h') &&  LangCmpOpt("-hidden",Tcl_GetString(objv[i]),length) == 0 ) {
	    /*
	     * -hidden is kept around for backwards compatibility with
	     * the dash patch, but -elide is the official option
	     */
	    searchElide = 1;
	} else if ((c == 'f') &&  LangCmpOpt("-forwards",Tcl_GetString(objv[i]),length) == 0 ) {
	    backwards = 0;
	} else if ((c == 'n') &&  LangCmpOpt("-nocase",Tcl_GetString(objv[i]),length) == 0 ) {
	    noCase = 1;
	} else if ((c == 'r') &&  LangCmpOpt("-regexp",Tcl_GetString(objv[i]),length) == 0 ) {
	    exact = 0;
	} else if ((c == 'h') &&  LangCmpOpt("-hidden",Tcl_GetString(objv[i]),length) == 0 ) {
	    searchElide = 1;
	} else if ((c == 'e') &&  LangCmpOpt("-elide",Tcl_GetString(objv[i]),length) == 0 ) {
	    searchElide = 1;
	} else if ((c == '-') &&  LangCmpOpt("--",Tcl_GetString(objv[i]),length) == 0 ) {
	    i++;
	    break;
	} else {
	    goto badSwitch;
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " search ?switches? pattern index ?stopIndex?\"",
		         NULL);
	return TCL_ERROR;
    }
    ...
    if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[i+1]), &index) != TCL_OK) {
	code = TCL_ERROR;
	goto done;
    }
    ...
    if (argsLeft == 1) {
	if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[i+2]), &stopIndex) != TCL_OK) {
	    code = TCL_ERROR;
	    goto done;
	}
	...
	if (matchByte >= 0) {
	    ...
	    if (varName != NULL) {
		Tcl_Obj *temp = Tcl_NewIntObj(numChars);
		if (Tcl_ObjSetVar2(interp, varName, NULL, temp, TCL_LEAVE_ERR_MSG)
			    == NULL) {
		        code = TCL_ERROR;
...
TkTextGetTabs(interp, tkwin, arg)
    ...
    Tk_Window tkwin;			/* Window in which the tabs will be
					 * used. */
    Tcl_Obj * arg;				/* Description of the tab stops.  See
					 * the text manual entry for details. */
{
	...
	if (Tk_GetPixels(interp, tkwin, Tcl_GetString(objv[i]), &tabPtr->location)
		!= TCL_OK) {
	    goto error;
	}
	tabArrayPtr->numTabs++;
	...
	i += 1;
	c = Tcl_GetString(objv[i])[0];
	if ((c == 'l') && (strncmp(Tcl_GetString(objv[i]), "left",
		strlen(Tcl_GetString(objv[i]))) == 0)) {
	    tabPtr->alignment = LEFT;
	} else if ((c == 'r') && (strncmp(Tcl_GetString(objv[i]), "right",
		strlen(Tcl_GetString(objv[i]))) == 0)) {
	    tabPtr->alignment = RIGHT;
	} else if ((c == 'c') && (strncmp(Tcl_GetString(objv[i]), "center",
		strlen(Tcl_GetString(objv[i]))) == 0)) {
	    tabPtr->alignment = CENTER;
	} else if ((c == 'n') && (strncmp(Tcl_GetString(objv[i]),
		"numeric", strlen(Tcl_GetString(objv[i]))) == 0)) {
	    tabPtr->alignment = NUMERIC;
	} else {
	    Tcl_AppendResult(interp, "bad tab alignment \"",
		    Tcl_GetString(objv[i]), "\": must be left, right, center, or numeric",
		             NULL);
	    goto error;
	}
    }
...
static int
TextDumpCmd(textPtr, interp, argc, objv)
    register TkText *textPtr;	/* Information about text widget. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings.  Someone else has already
				 * parsed this command enough to know that
				 * Tcl_GetString(objv[1]) is "dump". */
{
    TkTextIndex index1, index2;
    int arg;
...
#define TK_DUMP_ALL	(TK_DUMP_TEXT|TK_DUMP_MARK|TK_DUMP_TAG| \
    ...
    for (arg=2 ; Tcl_GetString(objv[arg]) !=          NULL ; arg++) {
	size_t len;
	if (Tcl_GetString(objv[arg])[0] != '-') {
	    break;
	}
	len = strlen(Tcl_GetString(objv[arg]));
	if (strncmp("-all", Tcl_GetString(objv[arg]), len) == 0) {
	    what = TK_DUMP_ALL;
	} else if (strncmp("-text", Tcl_GetString(objv[arg]), len) == 0) {
	    what |= TK_DUMP_TEXT;
	} else if (strncmp("-tag", Tcl_GetString(objv[arg]), len) == 0) {
	    what |= TK_DUMP_TAG;
	} else if (strncmp("-mark", Tcl_GetString(objv[arg]), len) == 0) {
	    what |= TK_DUMP_MARK;
	} else if (strncmp("-image", Tcl_GetString(objv[arg]), len) == 0) {
	    what |= TK_DUMP_IMG;
	} else if (strncmp("-window", Tcl_GetString(objv[arg]), len) == 0) {
	    what |= TK_DUMP_WIN;
	} else if (strncmp("-command", Tcl_GetString(objv[arg]), len) == 0) {
	    arg++;
	    if (arg >= argc) {
		Tcl_AppendResult(interp, "Usage: ", Tcl_GetString(objv[0]), " dump ?-all -image -text -mark -tag -window? ?-command script? index ?index2?", NULL);
		return TCL_ERROR;
	    }
    ...
    if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[arg]), &index1) != TCL_OK) {
	goto error;
    }
	...
	if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[arg]), &index2) != TCL_OK) {
	    goto error;
	}
	if (strncmp(Tcl_GetString(objv[arg]), "end", strlen(Tcl_GetString(objv[arg]))) == 0) {
	    atEnd = 1;
	}
    }
    if (TkTextIndexCmp(&index1, &index2) >= 0) {
    ...
    LangCallback *command;	/* Script to apply to the segment */
{
	...
	if ((what & TK_DUMP_TEXT) && (segPtr->typePtr == &tkTextCharType) &&
		...
		DumpSegment(interp, "mark", name, NULL, command, &index, what);
		...
		char *name = (eiPtr->name ==  NULL) ? "" : eiPtr->name;
                Tcl_Obj *obj;
		TkTextMakeByteIndex(textPtr->tree, lineno, offset, &index);
		obj = LangObjectObj( interp, eiPtr->name);
		DumpSegment(interp, "image", name, obj,
			command, &index, what);
 ...
 */
static int
DumpSegment(interp, key, value, arg, command, index, what)
    Tcl_Interp *interp;
    char *key;			/* Segment type key */
    char *value;		/* Segment value */
    Tcl_Obj *arg;		/* Segment as an Tcl_Obj * */
    ...
    int what;			/* Look for TK_DUMP_INDEX bit */
{
	...
	CONST Tcl_Obj **objv = LangAllocVec(4);
	char *list;
	LangSetString(objv+0, key);
	LangSetString(objv+1, value);
	LangSetString(objv+2, buffer);
	LangSetString(objv+3, NULL);
	list = Tcl_Merge(3, objv);
...
static int
TextEditCmd(textPtr, interp, argc, objv)
    TkText *textPtr;          /* Information about text widget. */
    Tcl_Interp *interp;       /* Current interpreter. */
    int argc;                 /* Number of arguments. */
    Tcl_Obj *CONST *objv;        /* Argument strings. */
{
    ...
    if (argc < 3) {
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " edit option ?arg arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    c = Tcl_GetString(objv[2])[0];
    length = strlen(Tcl_GetString(objv[2]));
    if ((c == 'm') && (strncmp(Tcl_GetString(objv[2]), "modified", length) == 0)) {
	if (argc == 3) {
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(textPtr->isDirty));
	} else if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " edit modified ?boolean?\"",          NULL);
	    return TCL_ERROR;
	} else {
	    XEvent event;
	    if (Tcl_GetBooleanFromObj(interp, objv[3], &setModified) != TCL_OK) {
		return TCL_ERROR;
            }
	    /*
    ...
    } else if ((c == 'r') && (strncmp(Tcl_GetString(objv[2]), "redo", length) == 0)
	    && (length >= 3)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " edit redo\"",          NULL);
	    return TCL_ERROR;
	}
    ...
    } else if ((c == 'r') && (strncmp(Tcl_GetString(objv[2]), "reset", length) == 0)
	    && (length >= 3)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " edit reset\"",          NULL);
	    return TCL_ERROR;
	}
        TkUndoClearStacks(textPtr->undoStack);
    } else if ((c == 's') && (strncmp(Tcl_GetString(objv[2]), "separator", length) == 0)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " edit separator\"",          NULL);
	    return TCL_ERROR;
	}
        TkUndoInsertUndoSeparator(textPtr->undoStack);
    } else if ((c == 'u') && (strncmp(Tcl_GetString(objv[2]), "undo", length) == 0)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " edit undo\"",          NULL);
	    return TCL_ERROR;
	}
	...
	Tcl_AppendResult(interp, "bad edit option \"", Tcl_GetString(objv[2]),
