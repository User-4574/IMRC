...
static int
TestwineventCmd(clientData, interp, argc, objv)
    ClientData clientData;		/* Main window for application. */
    Tcl_Interp *interp;			/* Current interpreter. */
    ...
    Tcl_Obj *CONST *objv;			/* Argument strings. */
{
    ...
    if ((argc == 3) && (strcmp(Tcl_GetString(objv[1]), "debug") == 0)) {
	int b;
...
#if 0
    TkpScanWindowId(interp, Tcl_GetString(objv[1]), &id);
    if (
#ifdef _WIN64
...
#endif
    hwnd = (HWND) strtol(Tcl_GetString(objv[1]), &rest, 0);
    if (rest == Tcl_GetString(objv[1])) {
	hwnd = FindWindow(NULL, Tcl_GetString(objv[1]));
	if (hwnd == NULL) {
    ...
    if (message < 0) {
	message = strtol(Tcl_GetString(objv[3]), NULL, 0);
    }
    wParam = 0;
    lParam = 0;
