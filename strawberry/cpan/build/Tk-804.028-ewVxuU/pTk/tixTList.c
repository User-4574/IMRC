...
static int		ConfigElement _ANSI_ARGS_((WidgetPtr wPtr,
			    ListEntry *chPtr, int argc, Tcl_Obj *CONST *objv,
...
static int		Tix_TLGetNeighbor _ANSI_ARGS_((
			    ...
			    int type, int argc, Tcl_Obj *CONST *objv));
 ...
 */
int
Tix_TListCmd(clientData, interp, argc, objv)
    ClientData clientData;
 ...
 */
static int
Tix_TLInsert(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (ConfigElement(wPtr, chPtr, argc-1, objv+1, 0, 1) != TCL_OK) {
	code = TCL_ERROR; goto done;
    }
 ...
 */
static int
Tix_TLIndex(clientData, interp, argc, objv)
 ...
 */
static int
Tix_TLInfo(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "active", len)==0) {
	return Tix_TLSpecialEntryInfo(wPtr, interp, wPtr->active);
    }
    else if (strncmp(Tcl_GetString(objv[0]), "down", len)==0) {
	return Tix_TLGetNeighbor(wPtr, interp, TIX_DOWN,  argc-1, objv+1);
    }
    else if (strncmp(Tcl_GetString(objv[0]), "left", len)==0) {
	return Tix_TLGetNeighbor(wPtr, interp, TIX_LEFT,  argc-1, objv+1);
    }
    else if (strncmp(Tcl_GetString(objv[0]), "right", len)==0) {
	return Tix_TLGetNeighbor(wPtr, interp, TIX_RIGHT, argc-1, objv+1);
    }
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "size", len)==0) {
	char buff[100];
    ...
    else if (strncmp(Tcl_GetString(objv[0]), "up", len)==0) {
	return Tix_TLGetNeighbor(wPtr, interp, TIX_UP,    argc-1, objv+1);
    }
    else {
...
static int Tix_TLGetNeighbor(wPtr, interp, type, argc, objv)
    WidgetPtr wPtr;
    Tcl_Interp *interp;		/* Current interpreter. */
    int type;
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (argc != 1) {
	Tix_ArgcError(interp, argc+3, objv-3, 3, "index");
    }
 ...
 */
static int
Tix_TLCGet(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_TLConfig(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_TLGeometryInfo(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_TLDelete(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (argc < 1 || argc > 2) {
	Tix_ArgcError(interp, argc+2, objv-2, 2, "from ?to?");
	code = TCL_ERROR;
	goto done;
    }

    if (Tix_TLGetFromTo(interp, wPtr, argc, objv, &fromPtr, &toPtr)!= TCL_OK) {
	code = TCL_ERROR;
	goto done;
    }
    if (fromPtr == NULL) {
 ...
 */
static int
Tix_TLEntryCget(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tix_TLGetFromTo(interp, wPtr, 1, objv, &chPtr, &dummy)
	!= TCL_OK) {
	return TCL_ERROR;
    }

    if (chPtr == NULL) {
	Tcl_AppendResult(interp, "list entry \"", Tcl_GetString(objv[0]),
	    "\" does not exist", NULL);
	return TCL_ERROR;
    }

    return Tix_ConfigureValue2(interp, wPtr->dispData.tkwin, (char *)chPtr,
	entryConfigSpecs, chPtr->iPtr, Tcl_GetString(objv[1]), 0);
}
 ...
 */
static int
Tix_TLEntryConfig(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	return Tix_ConfigureInfo2(interp, wPtr->dispData.tkwin,
	    (char*)chPtr, entryConfigSpecs, chPtr->iPtr,          NULL, 0);
    } else if (argc == 2) {
	return Tix_ConfigureInfo2(interp, wPtr->dispData.tkwin,
	    (char*)chPtr, entryConfigSpecs, chPtr->iPtr, (char *) Tcl_GetString(objv[1]), 0);
    } else {
 ...
 */
static int
Tix_TLNearest(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tcl_GetIntFromObj(interp, objv[0], &posn[0]) != TCL_OK) {
	return TCL_ERROR;
    }
    if (Tcl_GetIntFromObj(interp, objv[1], &posn[1]) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 */
static int
Tix_TLGetFromTo(interp, wPtr, argc, objv, fromPtr_ret, toPtr_ret)
    Tcl_Interp *interp;
    WidgetPtr wPtr;
    int argc;
    Tcl_Obj *CONST *objv;
...
static int
Tix_TLSelection(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	if (argc == 1) {
	    ...
	    if (Tix_TLGetFromTo(interp, wPtr, argc-1, objv+1, &fromPtr, &toPtr)
		    != TCL_OK) {
		code = TCL_ERROR;
		goto done;
	    }
	...
	if (argc != 2) {
	    Tix_ArgcError(interp, argc+2, objv-2, 3, "index");
	    code = TCL_ERROR;
	    goto done;
	}
	if (Tix_TLGetFromTo(interp, wPtr, argc-1, objv+1, &fromPtr, &toPtr)
 ...
 */
static int
Tix_TLSee(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (argc == 1) {
	if (Tix_TLGetFromTo(interp, wPtr, 1, objv, &chPtr, &dummy) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (chPtr != NULL) {
 ...
 */
static int
Tix_TLSetSite(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    else if (strncmp(Tcl_GetString(objv[-1]), "active", len)==0) {
 ...
 */
static int
Tix_TLView(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	(Tix_ScrollInfo*)&wPtr->scrollInfo[axis], argc, objv, 0) != TCL_OK) {
    ...
    int forced;
{
	...
	(char*)chPtr, entryConfigSpecs, chPtr->iPtr, argc, objv, flags,
