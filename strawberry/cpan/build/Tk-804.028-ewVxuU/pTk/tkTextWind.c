...
static int		EmbWinConfigure _ANSI_ARGS_((TkText *textPtr,
			    TkTextSegment *ewPtr, int argc, Tcl_Obj *CONST *objv));
				 ...
				 * Tcl_GetString(objv[1]) is "window". */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " window option ?arg arg ...?\"",          NULL);
	return TCL_ERROR;
    }
	...
	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " window cget index option\"",
		             NULL);
	    return TCL_ERROR;
	}
	...
	return Tk_ConfigureValue(interp, textPtr->tkwin, configSpecs,
		(char *) &ewPtr->body.ew, Tcl_GetString(objv[4]), 0);
	...
	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " window configure index ?option value ...?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[3]), &index) != TCL_OK) {
	...
	if (argc == 4) {
	    ...
	    return EmbWinConfigure(textPtr, ewPtr, argc-4, objv+4);
	}
	...
	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " window create index ?option value ...?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (TkTextGetIndex(interp, textPtr, Tcl_GetString(objv[3]), &index) != TCL_OK) {
	...
	if (EmbWinConfigure(textPtr, ewPtr, argc-4, objv+4) != TCL_OK) {
	    TkTextIndex index2;
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " window names\"",          NULL);
	    return TCL_ERROR;
	}
	...
	Tcl_AppendResult(interp, "bad window option \"", Tcl_GetString(objv[2]),
...
static int
EmbWinConfigure(textPtr, ewPtr, argc, objv)
				 ...
				 * options. */
{
    ...
    if (Tk_ConfigureWidget(textPtr->interp, textPtr->tkwin, configSpecs,
	    argc, objv, (char *) &ewPtr->body.ew, TK_CONFIG_ARGV_ONLY)
	    != TCL_OK) {
	return TCL_ERROR;
    }
