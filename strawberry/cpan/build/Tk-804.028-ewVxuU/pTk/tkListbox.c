    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
    ...
    Tcl_SetObjResult(interp,LangWidgetObj(interp,listPtr->tkwin));
    return TCL_OK;
}
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
	...
	if (listPtr->listVarName != NULL) {
		...
		if (Tcl_ObjSetVar2(interp, listPtr->listVarName,          NULL,
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (!(listPtr->flags & LISTBOX_DELETED)) {
	    ...
	    Lang_DeleteWidget(listPtr->interp, listPtr->widgetCmd);
	    if (listPtr->setGrid) {
		Tk_UnsetGrid(listPtr->tkwin);
	    }
    ...
    int flags;                  /* Information about what happened. */
{
	...
	if ((flags & TCL_TRACE_DESTROYED) && !(flags & TCL_INTERP_DESTROYED)) {
	    Tcl_ObjSetVar2(interp, listPtr->listVarName,
		             NULL, listPtr->listObj, TCL_GLOBAL_ONLY);
	...
	if (Tcl_ListObjLength(listPtr->interp, varListObj, &i) != TCL_OK) {
	    Tcl_ObjSetVar2(interp, listPtr->listVarName,          NULL,
