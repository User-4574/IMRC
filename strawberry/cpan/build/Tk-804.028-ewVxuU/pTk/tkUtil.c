...
int
Tk_GetScrollInfo(interp, argc, objv, dblPtr, intPtr)
					 ...
					 * or lines to scroll, if any. */
{
    ...
    if ((c == 'm') && (strncmp(Tcl_GetString(objv[2]), "moveto", length) == 0)) {
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " ", Tcl_GetString(objv[1]), " moveto fraction\"",
		             NULL);
    ...
    } else if ((c == 's') && (strncmp(Tcl_GetString(objv[2]), "scroll", length) == 0)) {

	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " ", Tcl_GetString(objv[1]), " scroll number units|pages\"",
		             NULL);
	    return TK_SCROLL_ERROR;
	}
	...
	if ((c == 'p') && (strncmp(Tcl_GetString(objv[4]), "pages", length) == 0)) {
	    return TK_SCROLL_PAGES;
	} else if ((c == 'u') && (strncmp(Tcl_GetString(objv[4]), "units", length) == 0)) {

	    return TK_SCROLL_UNITS;
	} else {
	    Tcl_AppendResult(interp, "bad argument \"", Tcl_GetString(objv[4]),
		    "\": must be units or pages",          NULL);
	    return TK_SCROLL_ERROR;
	}
    }
    Tcl_AppendResult(interp, "unknown option \"", Tcl_GetString(objv[2]),
    ...
    Tcl_Obj *keyObjPtr;		/* String key to find in lookup table. */
{
    ...
    if ((TclObjGetType(keyObjPtr) == &tkStateKeyObjType)
	    && (TclObjInternal(keyObjPtr)->twoPtrValue.ptr1 == (VOID *) mapPtr)) {
	return (int) TclObjInternal(keyObjPtr)->twoPtrValue.ptr2;
    }
	...
	if (strcmp(key, mPtr->strKey) == 0) {
	    ...
	    TclObjInternal(keyObjPtr)->twoPtrValue.ptr1 = (VOID *) mapPtr;
	    TclObjInternal(keyObjPtr)->twoPtrValue.ptr2 = (VOID *) mPtr->numKey;
	    TclObjSetType(keyObjPtr,&tkStateKeyObjType);
	    return mPtr->numKey;
	}
    }
    if (interp != NULL) {
