    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
	...
	if (focusWinPtr != NULL) {
	    Tcl_SetObjResult(interp,LangWidgetObj(interp,(Tk_Window)(focusWinPtr)));
	}
	return TCL_OK;
    }
    ...
    switch (index) {
	    ...
	    if (newPtr != NULL) {
		Tcl_SetObjResult(interp,LangWidgetObj(interp,(Tk_Window)(newPtr)));
	    }
	    break;
	}
        case 1: {        /* -force */
		...
		if (topLevelPtr->flags & TK_TOP_HIERARCHY) {
		    for (tlFocusPtr = newPtr->mainPtr->tlFocusPtr;
			    ...
			    Tcl_SetObjResult(interp,LangWidgetObj(interp,(Tk_Window)(tlFocusPtr->focusWinPtr)));


			    return TCL_OK;
			}
		    }
		    Tcl_SetObjResult(interp,LangWidgetObj(interp,(Tk_Window)(topLevelPtr)));
		    return TCL_OK;
		}
	    }
	    break;
