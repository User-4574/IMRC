...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_PIXELS, "-borderwidth", "borderWidth",          NULL,
    ...
    {TK_CONFIG_BOOLEAN, "-showbackground",          NULL,          NULL,
...
static int		ImgCmpConfigureMaster _ANSI_ARGS_((
			    CmpMaster *masterPtr, int argc, Tcl_Obj *CONST *objv,
			    int flags));
				 ...
				 * it will be returned in later callbacks. */
{
     ...
     */
    if (argc > 10) {
	objv = (char **) ckalloc(argc * sizeof(char *));
    }
    for (i = 0; i < argc; i++) {
	LangSetString(objv+i, TixGetStringFromObj(objv[i], NULL));
    }
#endif
 ...
 */
static int
ImgCmpConfigureMaster(masterPtr, argc, objv, flags)
				 ...
				 * such as TK_CONFIG_ARGV_ONLY. */
{
	...
	Tcl_AppendResult(masterPtr->interp, "value missing for option \"",
	    Tcl_GetString(objv[argc-1]), "\"", NULL);
	return TCL_ERROR;
    }
    for (i=0; i<argc; i+=2) {
	size_t length = strlen(Tcl_GetString(objv[i]));
	if (strncmp(Tcl_GetString(objv[i]), "-window", length) == 0) {
	    masterPtr->tkwin = Tk_NameToWindow(masterPtr->interp, Tcl_GetString(objv[i+1]),
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	    configSpecs, argc, objv, (char *) masterPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 */
static int
ImgCmpCmd(clientData, interp, argc, objv)
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if ((c == 'a') && (strncmp(Tcl_GetString(objv[1]), "add", length) == 0)) {
	if (argc < 3) {
	    return Tix_ArgcError(interp, argc, objv, 2,
		"type ?option value? ...");
	}
	c = Tcl_GetString(objv[2])[0];
	length = strlen(Tcl_GetString(objv[2]));

	if ((c == 'l') && (strncmp(Tcl_GetString(objv[2]), "line", length) == 0)) {
	    CmpLine * newLine;

	    newLine = AddNewLine(masterPtr, argc-3, objv+3);
	    if (newLine == NULL) {
		return TCL_ERROR;
	    }
	}
	    ...
	    if ((c == 'b') && (strncmp(Tcl_GetString(objv[2]), "bitmap", length) == 0)) {
	    ...
	    else if ((c == 'i') && (strncmp(Tcl_GetString(objv[2]), "image", length) == 0)) {
	    ...
	    else if ((c == 's') && (strncmp(Tcl_GetString(objv[2]), "space", length) == 0)) {
	    ...
	    else if ((c == 't') && (strncmp(Tcl_GetString(objv[2]), "text", length) == 0)) {
		...
		Tcl_AppendResult(interp, "unknown option \"",
		    Tcl_GetString(objv[2]), "\", must be bitmap, image, line, ",
	...
	if (argc == 2) {
	    ...
	    int i;
	    for (i=2; i<argc-2; i++) {
		length = strlen(Tcl_GetString(objv[i]));
		if (strncmp(Tcl_GetString(objv[i]), "-window", length) == 0) {
	    ...
	    code = ImgCmpConfigureMaster(masterPtr, argc-2, objv+2,
		TK_CONFIG_ARGV_ONLY);
	}
	return code;
    } else if ((c == 'i') && (strncmp(Tcl_GetString(objv[1]), "itemconfigure", length)== 0)) {
	Tcl_AppendResult(interp, "unimplemented", NULL);
	return TCL_ERROR;
    } else if ((c == 'l') && (strncmp(Tcl_GetString(objv[1]), "lineconfigure", length)== 0)) {
	Tcl_AppendResult(interp, "unimplemented", NULL);
	return TCL_ERROR;
    } else {
 ...
 */
CmpLine *
AddNewLine(masterPtr, argc, objv)
    CmpMaster *masterPtr;
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	    lineConfigSpecs, argc, objv, (char *) lPtr,
 ...
 */
CmpBitmapItem *
AddNewBitmap(masterPtr, line, argc, objv)
    CmpMaster *masterPtr;
    CmpLine *line;
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	    bitmapConfigSpecs, argc, objv, (char *) p.bitmap,
	    TK_CONFIG_ARGV_ONLY) != TCL_OK) {
	goto error;
    }
 ...
 */
CmpImageItem *
AddNewImage(masterPtr, line, argc, objv)
    CmpMaster *masterPtr;
    CmpLine *line;
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	    imageConfigSpecs, argc, objv, (char *) p.image,
	    TK_CONFIG_ARGV_ONLY) != TCL_OK) {
	goto error;
    }
 ...
 */
CmpSpaceItem *
AddNewSpace(masterPtr, line, argc, objv)
    CmpMaster *masterPtr;
    CmpLine *line;
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	spaceConfigSpecs, argc, objv, (char *)p.space,
	TK_CONFIG_ARGV_ONLY) != TCL_OK) {
	goto error;
    }
 ...
 */
CmpTextItem *
AddNewText(masterPtr, line, argc, objv)
    CmpMaster *masterPtr;
    CmpLine *line;
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	textConfigSpecs, argc, objv, (char *) p.text,
	TK_CONFIG_ARGV_ONLY) != TCL_OK) {
