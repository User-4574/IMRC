...
int Tix_GrConfigSize(interp, wPtr, argc, objv, sizePtr, argcErrorMsg,
	changed_ret)
    Tcl_Interp *interp;
    WidgetPtr wPtr;
    int argc;
    ...
    int *changed_ret;
{
	...
	if (strncmp("-size", Tcl_GetString(objv[i]), strlen(Tcl_GetString(objv[i]))) == 0) {
	    if (strcmp(Tcl_GetString(objv[i+1]), "auto")==0) {
		newSize.sizeType  = TIX_GR_AUTO;
		newSize.sizeValue = 0;
	    }
	    else if (strcmp(Tcl_GetString(objv[i+1]), "default")==0) {
		newSize.sizeType  = TIX_GR_DEFAULT;
		newSize.sizeValue = 0;
	    }
	    else if (Tk_GetPixels(interp, wPtr->dispData.tkwin, Tcl_GetString(objv[i+1]),
		 &pixels) == TCL_OK) {
		...
		if (Tix_GetChars(interp, Tcl_GetString(objv[i+1]), &chars) == TCL_OK) {
	...
	else if (strcmp("-pad0", Tcl_GetString(objv[i])) == 0) {
	    if (Tk_GetPixels(interp, wPtr->dispData.tkwin, Tcl_GetString(objv[i+1]),
		 &pixels) == TCL_OK) {
	...
	else if (strcmp("-pad1", Tcl_GetString(objv[i])) == 0) {
	    if (Tk_GetPixels(interp, wPtr->dispData.tkwin, Tcl_GetString(objv[i+1]),
		 &pixels) == TCL_OK) {
	    ...
	    Tcl_AppendResult(interp, "Unknown option \"", Tcl_GetString(objv[i]),
