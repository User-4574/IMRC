...
Tk_ConfigSpec tkpScrollbarConfigSpecs[] = {
	...
	TK_CONFIG_NULL_OK},
#ifdef NOT_YET
    {TK_CONFIG_CUSTOM, "-tile", "tile", "Tile",          NULL,
...
static int		ConfigureScrollbar _ANSI_ARGS_((Tcl_Interp *interp,
			    TkScrollbar *scrollPtr, int argc, Tcl_Obj *CONST *objv,
			    int flags));
...
int
Tk_ScrollbarCmd(clientData, interp, argc, objv)
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    scrollPtr->widgetCmd = Lang_CreateWidget(interp,scrollPtr->tkwin, ScrollbarWidgetCmd, (ClientData) scrollPtr, ScrollbarCmdDeletedProc);
    ...
    if (ConfigureScrollbar(interp, scrollPtr, argc-2, objv+2, 0) != TCL_OK) {
	Tk_DestroyWindow(scrollPtr->tkwin);
	return TCL_ERROR;
    }

    Tcl_SetObjResult(interp,LangWidgetObj(interp,scrollPtr->tkwin));
    return TCL_OK;
}
...
static int
ScrollbarWidgetCmd(clientData, interp, argc, objv)
    ...
    Tcl_Obj *CONST *objv;			/* Argument strings. */
{
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " activate element\"",          NULL);
	    goto error;
	}
	...
	result = Tk_ConfigureValue(interp, scrollPtr->tkwin,
		tkpScrollbarConfigSpecs, (char *) scrollPtr, Tcl_GetString(objv[2]), 0);
	...
	if (argc == 2) {
	    ...
	    result = ConfigureScrollbar(interp, scrollPtr, argc-2, objv+2,
		    TK_CONFIG_ARGV_ONLY);
	}
    } else if ((c == 'd') && (strncmp(Tcl_GetString(objv[1]), "delta", length) == 0)) {
	...
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " delta xDelta yDelta\"",          NULL);
	    goto error;
	}
    ...
    } else if ((c == 'f') && (strncmp(Tcl_GetString(objv[1]), "fraction", length) == 0)) {
	int x, y, pos, length;
	double fraction;
	char buf[TCL_DOUBLE_SPACE];
	...
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " fraction x y\"",          NULL);
	    goto error;
	}
    ...
    } else if ((c == 's') && (strncmp(Tcl_GetString(objv[1]), "set", length) == 0)) {
	...
	if (argc == 4) {
	    ...
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    Tcl_GetString(objv[0]), " set firstFraction lastFraction\" or \"",
...
static int
ConfigureScrollbar(interp, scrollPtr, argc, objv, flags)
					 ...
					 * Tk_ConfigureWidget. */
{
    if (Tk_ConfigureWidget(interp, scrollPtr->tkwin, tkpScrollbarConfigSpecs,
	    argc, objv, (char *) scrollPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (scrollPtr->tkwin != NULL) {
	    ...
	    Lang_DeleteWidget(scrollPtr->interp, scrollPtr->widgetCmd);
