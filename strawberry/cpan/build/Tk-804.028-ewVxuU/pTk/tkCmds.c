...
{
    static CONST char *bellOptions[] = {"-displayof", "-nice",          NULL};
    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
    ...
    switch ((enum options) index) {
		...
		Tcl_SetResult(interp, "appname not accessible in a safe interpreter", TCL_STATIC);
	...
	case TK_SCALING: {
	    ...
	    if (Tcl_IsSafe(interp)) {
		Tcl_SetResult(interp, "scaling not accessible in a safe interpreter", TCL_STATIC);
	...
	case TK_USE_IM: {
	    ...
	    if (Tcl_IsSafe(interp)) {
		Tcl_SetResult(interp, "useinputmethods not accessible in a safe interpreter", TCL_STATIC);
