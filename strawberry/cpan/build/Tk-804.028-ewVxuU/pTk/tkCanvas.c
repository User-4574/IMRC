...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_CUSTOM, "-disabledtile", "disabledtile", "Tile",          NULL,
    ...
    Tcl_Obj *CONST objv[];      /* Argument objects. */
{
    ...
    Tcl_SetObjResult(interp,LangWidgetObj(interp,canvasPtr->tkwin));
    return TCL_OK;
    ...
    XEvent *eventPtr;           /* Information about event. */
{
	...
	if (canvasPtr->tkwin != NULL) {
	    Lang_DeleteWidget(canvasPtr->interp, canvasPtr->widgetCmd);
...
FindItems(interp, canvasPtr, argc, objv, newTag, first, searchPtrPtr)
    ...
    TkCanvas *canvasPtr;                /* Canvas whose items are to be
					 * searched. */
    int argc;                           /* Number of entries in objv.  Must be
					 * greater than zero. */
...
#endif /* not USE_OLD_TAG_SEARCH */
{
	...
	if (argc > first+3) {
		...
		Tcl_AppendResult(interp, "can't have negative halo value \"",
			Tcl_GetString(objv[first+3]), "\"",          NULL);
		return TCL_ERROR;
	    }
	} else {
	    halo = 0.0;
