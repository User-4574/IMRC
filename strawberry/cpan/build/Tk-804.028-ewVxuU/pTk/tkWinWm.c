    ...
    TkWindow *winPtr;		/* Top-level window to redecorate. */
{
	...
	if (!wmPtr->wrapper) {
	    Tcl_Panic("UpdateWrapper: Failed to create container");
	}
#ifdef _WIN64
    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
	...
	Tcl_AppendResult(interp, "-disabled ", buf,          NULL);
	sprintf(buf, "%d", ((exStyle & WS_EX_TOOLWINDOW) != 0));
	Tcl_AppendResult(interp, " -toolwindow ", buf,          NULL);
	sprintf(buf, "%d", ((exStyle & WS_EX_TOPMOST) != 0));
	Tcl_AppendResult(interp, " -topmost ", buf,          NULL);
	return TCL_OK;
    }
    for (i = 3; i < objc; i += 2) {
    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
    ...
    if (objc == 3) {
	if (wmPtr->hints.flags & IconMaskHint) {
	    Tcl_SetResult(interp, (char *) Tk_NameOfBitmap(winPtr->display, wmPtr->hints.icon_mask), TCL_STATIC);
    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
    ...
    if (objc == 3) {
	if (wmPtr->icon != NULL) {
	    Tcl_SetObjResult(interp,LangWidgetObj(interp,wmPtr->icon));
	}
	return TCL_OK;
    }
    if (*Tcl_GetString(objv[3]) == '\0') {
    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
    ...
    if (objc == 3) {
	Tcl_SetResult(interp, (char *) ((wmPtr->title != NULL) ? wmPtr->title : winPtr->nameUid), TCL_STATIC);
...
Tcl_Obj *CONST objv[];
{
	...
	if ((objc != 3) && (objc != 4)) {
	    Tcl_AppendResult(interp, "wrong # arguments: must be \"",
		    Tcl_GetString(objv[0]), " iconimage window ?image?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (objc == 3) {
