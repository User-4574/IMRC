...
TclInitNotifier()
{
    ...
    tsdPtr->clientData = TkeventVptr->V_Tcl_InitNotifier();
...
TclFinalizeNotifier()
{
    ...
    if (TkeventVptr->V_Tcl_FinalizeNotifier) {
	TkeventVptr->V_Tcl_FinalizeNotifier(tsdPtr->clientData);
    }
    ...
    Tcl_NotifierProcs *notifierProcPtr;
{
    ...
    TkeventVptr->V_Tcl_CreateFileHandler = notifierProcPtr->createFileHandlerProc;
    TkeventVptr->V_Tcl_DeleteFileHandler = notifierProcPtr->deleteFileHandlerProc;
#endif
    TkeventVptr->V_Tcl_SetTimer = notifierProcPtr->setTimerProc;
    TkeventVptr->V_Tcl_WaitForEvent = notifierProcPtr->waitForEventProc;
    TkeventVptr->V_Tcl_InitNotifier = notifierProcPtr->initNotifierProc;
    TkeventVptr->V_Tcl_FinalizeNotifier = notifierProcPtr->finalizeNotifierProc;
    TkeventVptr->V_Tcl_AlertNotifier = notifierProcPtr->alertNotifierProc;
    TkeventVptr->V_Tcl_ServiceModeHook = notifierProcPtr->serviceModeHookProc;
}
				 ...
				 * TCL_SERVICE_NONE */
{
    ...
    if (TkeventVptr->V_Tcl_ServiceModeHook) {
	TkeventVptr->V_Tcl_ServiceModeHook(mode);
    }
    return oldMode;
}
    ...
    Tcl_ThreadId threadId;	/* Identifier for thread to use. */
{
	...
	if (tsdPtr->threadId == threadId) {
	    if (TkeventVptr->V_Tcl_AlertNotifier) {
		TkeventVptr->V_Tcl_AlertNotifier(tsdPtr->clientData);
	    }
	    break;
	}
    }
