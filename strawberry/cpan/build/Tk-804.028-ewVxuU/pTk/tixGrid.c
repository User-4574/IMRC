...
static int		ConfigElement _ANSI_ARGS_((WidgetPtr wPtr,
			    TixGrEntry *chPtr, int argc, Tcl_Obj *CONST *objv,
...
static int		TranslateFromTo _ANSI_ARGS_((Tcl_Interp * interp,
			    WidgetPtr wPtr, int argc, Tcl_Obj *CONST *objv, int *from,
			    int * to, int *which));
 ...
 */
int
Tix_GridCmd(clientData, interp, argc, objv)
    ClientData clientData;
 ...
 */
static int
Tix_GrBdType(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (argc != 2 && argc != 4) {
	return Tix_ArgcError(interp, argc+2, objv-2, 2,
	    "x y ?xbdWidth ybdWidth?");
    }

    if (Tcl_GetIntFromObj(interp, objv[0], &screenPos[0]) != TCL_OK) {
	return TCL_ERROR;
    }
    if (Tcl_GetIntFromObj(interp, objv[1], &screenPos[1]) != TCL_OK) {
	return TCL_ERROR;
    }
    if (argc == 4) {
	if (Tcl_GetIntFromObj(interp, objv[2], &bdWidth[0]) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (Tcl_GetIntFromObj(interp, objv[3], &bdWidth[1]) != TCL_OK) {
	    return TCL_ERROR;
	}
    } else {
	bdWidth[0] = -1;
 ...
 */
static int
Tix_GrSet(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (ConfigElement(wPtr, chPtr, argc-2, objv+2, 0, 1) != TCL_OK) {
...
static int
Tix_GrUnset(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_GrCGet(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_GrConfig(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_GrDelete(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (TranslateFromTo(interp, wPtr, argc, objv, &from, &to, &which)!=TCL_OK){
	return TCL_ERROR;
    }
 ...
 */
static int
Tix_GrEdit(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;	/* Argument strings. */
{
	...
	if (argc != 3) {
		...
		Tcl_GetString(objv[-2]), " edit set x y", NULL);
	}
    ...
    } else if (strncmp(Tcl_GetString(objv[0]), "apply", len) == 0) {
	if (argc != 1) {
	    Tcl_AppendResult(interp, "wrong # of arguments, must be: ",
		Tcl_GetString(objv[-2]), " edit apply", NULL);
	}
	obj  = LangWidgetObj(interp,wPtr->dispData.tkwin);
 ...
 */
static int
Tix_GrEntryCget(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    chPtr = Tix_GrFindElem(interp, wPtr, x, y);
    if (!chPtr) {
	Tcl_AppendResult(interp, "entry \"", Tcl_GetString(objv[0]), ",", Tcl_GetString(objv[1]),
	    "\" does not exist", NULL);
	return TCL_ERROR;
    }

    return Tix_ConfigureValue2(interp, wPtr->dispData.tkwin, (char *)chPtr,
	entryConfigSpecs, chPtr->iPtr, Tcl_GetString(objv[2]), 0);
}
 ...
 */
static int
Tix_GrEntryConfig(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	return Tix_ConfigureInfo2(interp, wPtr->dispData.tkwin,
	    (char*)chPtr, entryConfigSpecs, chPtr->iPtr, (char *) Tcl_GetString(objv[2]), 0);
    } else {
	return ConfigElement(wPtr, chPtr, argc-2, objv+2,
 ...
 */
static int
Tix_GrGeometryInfo(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_GrIndex(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_GrInfo(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (strncmp(Tcl_GetString(objv[0]), "bbox", len)==0) {
	if (argc != 3) {
	    return Tix_ArgcError(interp, argc+2, objv-2, 3, "x y");
	}
	if (TixGridDataGetIndex(interp, wPtr, objv[1], objv[2], &x, &y)
		!=TCL_OK) {
	    return TCL_ERROR;
 ...
 */
static int
Tix_GrMove(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (TranslateFromTo(interp, wPtr, 3, objv, &from, &to, &which)!=TCL_OK){
	return TCL_ERROR;
    }
    if (Tcl_GetIntFromObj(interp, objv[3], &by) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 */
static int
Tix_GrNearest(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_GrSetSite(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    len = strlen(Tcl_GetString(objv[0]));
    if (strncmp(Tcl_GetString(objv[0]), "get", len)==0) {
 ...
 */
static int
Tix_GrSee(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_GrView(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	if (Tcl_GetIntFromObj(interp, objv[0], &offset) == TCL_OK) {
...
static int
TranslateFromTo(interp, wPtr, argc, objv, from, to, which)
    Tcl_Interp * interp;
    WidgetPtr wPtr;
    int argc;
    Tcl_Obj *CONST *objv;
    ...
    int * which;
{
    ...
    if (strncmp(Tcl_GetString(objv[0]), "row", len) == 0) {
	*which = 1;
    ...
    } else if (strncmp(Tcl_GetString(objv[0]), "column", len) == 0) {
	*which = 0;
