...
static HListElement *	Tix_HLGetColumn _ANSI_ARGS_((Tcl_Interp *interp,
			    WidgetPtr wPtr, Tcl_Obj *CONST *objv, int * column_ret,
			    int mustExist));
...
static HListElement *
Tix_HLGetColumn(interp, wPtr, objv, column_ret, mustExist)
    Tcl_Interp *interp;
    WidgetPtr wPtr;
    Tcl_Obj *CONST *objv;
    int * column_ret;
    int mustExist;
{
    ...
    if (Tcl_GetIntFromObj(interp, objv[1], &column) != TCL_OK) {
	return NULL;
    }
    if (column >= wPtr->numColumns || column < 0) {
	Tcl_AppendResult(interp, "Column \"", Tcl_GetString(objv[1]),
	    "\" does not exist",          NULL);
	return NULL;
    }
    if (mustExist && chPtr->col[column].iPtr == NULL) {
	Tcl_AppendResult(interp, "entry \"", Tcl_GetString(objv[0]),
	    "\" does not have an item at column ", Tcl_GetString(objv[1]), NULL);
	return NULL;
    }
 ...
 */
int
Tix_HLItem(clientData, interp, argc, objv)
 ...
 */
static int
Tix_HLItemCGet(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if ((chPtr=Tix_HLGetColumn(interp, wPtr, objv, &column, 1)) == NULL) {
	return TCL_ERROR;
    }

    return Tk_ConfigureValue(interp, wPtr->dispData.tkwin,
	chPtr->col[column].iPtr->base.diTypePtr->itemConfigSpecs,
	(char *)chPtr->col[column].iPtr, Tcl_GetString(objv[2]), 0);
}
 ...
 */
static int
Tix_HLItemConfig(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	return Tix_DItemConfigure(chPtr->col[column].iPtr,
	    argc-2, objv+2, TK_CONFIG_ARGV_ONLY);
    }
}
 ...
 */
static int
Tix_HLItemCreate(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if ((chPtr=Tix_HLGetColumn(interp, wPtr, objv, &column, 0)) == NULL) {
	return TCL_ERROR;
    }
    ...
    if (Tix_DItemConfigure(iPtr, argc-2, objv+2, 0) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 */
static int
Tix_HLItemDelete(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (column == 0) {
	Tcl_AppendResult(interp,"Cannot delete item at column 0",         NULL);
	return TCL_ERROR;
    }
 ...
 */
static int
Tix_HLItemExists(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
int
Tix_HLColumn(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
 ...
 */
static int
Tix_HLColWidth(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (Tcl_GetIntFromObj(interp, objv[0], &column) != TCL_OK) {
	return TCL_ERROR;
    }
    if (column >= wPtr->numColumns || column < 0) {
	Tcl_AppendResult(interp, "Column \"", Tcl_GetString(objv[0]),
	    "\" does not exist",          NULL);
	return TCL_ERROR;
    }
    if (argc == 1) {		/* Query */
	...
	if (Tk_GetPixels(interp, wPtr->dispData.tkwin, Tcl_GetString(objv[1]),
		 &newWidth) != TCL_OK) {
	    return TCL_ERROR;
	}
 	if (newWidth < 0) {
    ...
    else if (argc == 3 && strcmp(Tcl_GetString(objv[1]), "-char")==0) {
	if (Tcl_GetString(objv[2])[0] == '\0') {
	    newWidth = UNINITIALIZED;
	    goto setwidth;
	}
	if (Tcl_GetIntFromObj(interp, objv[2], &newWidth) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (newWidth < 0) {
	    newWidth = 0;
	...
	return Tix_ArgcError(interp, argc+3, objv-3, 3,
	    "column ?-char? ?size?");
    }
