 ...
 *----------------------------------------------------------------------
 */
int TixFm_Info(clientData, interp, argc, objv)
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    clientPtr = TixFm_FindClientPtrByName(interp, Tcl_GetString(objv[0]), topLevel);
    if (clientPtr == NULL) {
	return TCL_ERROR;
    }
	...
	for (i=0; i<2; i++) {
		...
		if (strcmp(Tcl_GetString(objv[1]), sideNames[i][j]) == 0) {
		    AttachInfo(interp, clientPtr, i, j);
		    return TCL_OK;
		}

		/* Do you want to know padding? */
		if (strcmp(Tcl_GetString(objv[1]), padNames[i][j]) == 0) {
		    Tcl_SetIntObj(Tcl_GetObjResult(interp),clientPtr->pad[i][j]);
		    return TCL_OK;
		}
	    }
	}
	Tcl_AppendResult(interp, "Unknown option \"", Tcl_GetString(objv[1]), "\"", NULL);
	return TCL_ERROR;
    }
    ...
    Tcl_Obj * avalue;
{
	...
	break;
      ...
      case 'n':		/* none */
	if (argc == 1 && strcmp(Tcl_GetString(objv[0]), "none") == 0) {
	...
	if (Tk_GetPixels(interp, topLevel, Tcl_GetString(objv[0]), &offset) != TCL_OK) {
	    goto error;
	}

	clientPtr->attType[axis][which]      = ATT_GRID;
	clientPtr->off    [axis][which]      = offset;
	if (offset < 0 || (offset == 0 && strcmp(Tcl_GetString(objv[0]), "-0") ==0)) {
    ...
    if (argc  == 2+delta) {
	if (Tk_GetPixels(interp, topLevel, Tcl_GetString(objv[1+delta]), &offset) != TCL_OK) {
	    goto error;
	}
    	clientPtr->off[axis][which] = offset;
    } else {
...
int TixFm_Configure(clientPtr, topLevel, interp, argc, objv)
    FormInfo *clientPtr;
    Tk_Window topLevel;
    Tcl_Interp* interp;
    int argc;
    Tcl_Obj *CONST *objv;
{
	...
	if (strcmp(Tcl_GetString(objv[flag]), "-in") == 0) {
	    /* Reset the parent of the widget
	     */
	    Tcl_AppendResult(interp,
		"\"-in \" must be the first option given to tixForm", NULL);
	    return TCL_ERROR;
	} else if (strcmp(Tcl_GetString(objv[flag]), "-l") == 0) {
	    if (ConfigureAttachment(clientPtr, topLevel, interp,
		0, 0, objv[value]) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-left") == 0) {
	    if (ConfigureAttachment(clientPtr, topLevel, interp,
		0, 0, objv[value]) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-r") == 0) {
	    if (ConfigureAttachment(clientPtr, topLevel, interp,
		0, 1, objv[value]) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-right") == 0) {
	    if (ConfigureAttachment(clientPtr, topLevel, interp,
		0, 1, objv[value]) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-top") == 0) {
	    if (ConfigureAttachment(clientPtr, topLevel, interp,
	        1, 0, objv[value]) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-t") == 0) {
	    if (ConfigureAttachment(clientPtr, topLevel, interp,
	        1, 0, objv[value]) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-bottom") == 0) {
	    if (ConfigureAttachment(clientPtr, topLevel, interp,
		1, 1, objv[value]) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-b") == 0) {
	    if (ConfigureAttachment(clientPtr, topLevel, interp,
		1, 1, objv[value]) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-padx") == 0) {
	    if (ConfigurePadding(clientPtr, topLevel, interp,
  	        0, 0, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		0, 1, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-pady") == 0) {
	    if (ConfigurePadding(clientPtr,topLevel,interp,
		1, 0, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		1, 1, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-padleft") == 0) {
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		0, 0, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-lp") == 0) {
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		0, 0, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-padright")== 0){
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		0, 1, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-rp")== 0){
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		0, 1, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-padtop")== 0) {
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		1, 0, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-tp")== 0) {
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		1, 0, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]),"-padbottom")== 0){
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		1, 1, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]),"-bp")== 0){
	    if (ConfigurePadding(clientPtr, topLevel, interp,
		1, 1, Tcl_GetString(objv[value])) == TCL_ERROR) {

		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-leftspring") == 0) {
	    if (ConfigureSpring(clientPtr, topLevel, interp,
		0, 0, objv[value]) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-ls") == 0) {
	    if (ConfigureSpring(clientPtr, topLevel, interp,
		0, 0, objv[value]) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-rightspring") == 0) {
	    if (ConfigureSpring(clientPtr, topLevel, interp,
		0, 1, objv[value]) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-rs") == 0) {
	    if (ConfigureSpring(clientPtr, topLevel, interp,
		0, 1, objv[value]) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-topspring") == 0) {
	    if (ConfigureSpring(clientPtr, topLevel, interp,
		1, 0, objv[value]) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-ts") == 0) {
	    if (ConfigureSpring(clientPtr, topLevel, interp,
		1, 0, objv[value]) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-bottomspring") == 0) {
	    if (ConfigureSpring(clientPtr, topLevel, interp,
		1, 1, objv[value]) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-bs") == 0) {
	    if (ConfigureSpring(clientPtr, topLevel, interp,
		1, 1, objv[value]) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else if (strcmp(Tcl_GetString(objv[flag]), "-fill") == 0) {
	    if (ConfigureFill(clientPtr, topLevel, interp,
		Tcl_GetString(objv[value])) == TCL_ERROR) {
		return TCL_ERROR;
	    }
	} else {
