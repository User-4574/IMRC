 ...
 */
int
Tix_ItemStyleCmd(clientData, interp, argc, objv)
    ClientData clientData;
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (argc < 2) {
	return Tix_ArgcError(interp, argc, objv, 1,
	    "itemtype ?option value ...");
    }

    if ((diTypePtr=Tix_GetDItemType(interp, Tcl_GetString(objv[1]))) == NULL) {
	return TCL_ERROR;
    }
	...
	for (n=i=2; i<argc; i+=2) {
	    ...
	    if (strncmp(Tcl_GetString(objv[i]), "-refwindow", len) == 0) {
		if ((tkwin=Tk_NameToWindow(interp,Tcl_GetString(objv[i+1]),tkwin)) == NULL) {
		    return TCL_ERROR;
		}
		continue;
	    }
	    if (strncmp(Tcl_GetString(objv[i]), "-stylename", len) == 0) {
		styleName = Tcl_GetString(objv[i+1]);
		if (FindStyle(styleName, interp) != NULL) {
		    Tcl_AppendResult(interp, "style \"", Tcl_GetString(objv[i+1]),
			"\" already exist", NULL);
		    return TCL_ERROR;
		}
		continue;
    ...
    if (StyleConfigure(interp, stylePtr, argc-2, objv+2, 0) != TCL_OK) {
...
static int
StyleCmd(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int argc;
    Tcl_Obj *CONST *objv;
 ...
 */
static int
StyleCGetCmd(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	(char *)stylePtr, Tcl_GetString(objv[0]), 0);
}
 ...
 */
static int
StyleConfigCmd(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	return StyleConfigure(interp, stylePtr, argc, objv,
	    TK_CONFIG_ARGV_ONLY);
    }
}
 ...
 */
static int
StyleDeleteCmd(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
...
static int
StyleConfigure(interp, stylePtr, argc, objv, flags)
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (diTypePtr->styleConfigureProc(stylePtr, argc, objv, flags) != TCL_OK) {
	return TCL_ERROR;
    }
    return TCL_OK;
}
