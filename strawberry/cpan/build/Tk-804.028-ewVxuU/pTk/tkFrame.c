...
static Tk_OptionSpec labelframeOptSpec[] = {
    ...
    {TK_OPTION_WINDOW, "-labelwidget", "labelWidget", "LabelWidget",
	         NULL, -1, Tk_Offset(Labelframe, labelWin),
	TK_OPTION_NULL_OK, 0, 0},
    {TK_OPTION_RELIEF, "-relief", "relief", "Relief",
...
int
TkCreateFrame(clientData, interp, argc, objv, toplevel, appName)
    ClientData clientData;      /* Either NULL or pointer to option table. */
    Tcl_Interp *interp;         /* Current interpreter. */
				 ...
				 * new application. */
{
	...
	c = arg[1];
	if ((c == 'c') &&  LangCmpOpt("-class",arg,length) == 0
		&& (length >= 3)) {
	    className = Tcl_GetString(objv[i+1]);
	} else if ((c == 'c') &&  LangCmpOpt("-colormap",arg,length) == 0 ) {
	    ...
	    useOption = objv[i+1];
	} else if ((c == 'v') &&  LangCmpOpt("-visual",arg,length) == 0 ) {
    ...
    Tcl_SetObjResult(interp,LangWidgetObj(interp,new));
    return TCL_OK;
