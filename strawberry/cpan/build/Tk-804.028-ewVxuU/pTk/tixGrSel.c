...
int
Tix_GrSelection(clientData, interp, argc, objv)
...
static int
Tix_GrSelIncludes(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (argc != 2 && argc != 4) {
	return Tix_ArgcError(interp, argc+2, objv-2, 2, "x1 y1 ?x2 y2?");
    }
    if (Tcl_GetIntFromObj(interp, objv[0], &col) != TCL_OK) {
	return TCL_ERROR;
    }
    if (Tcl_GetIntFromObj(interp, objv[1], &row) != TCL_OK) {
	return TCL_ERROR;
    }
    if (argc == 2)
     {
    ...
    else
     {
      int row2;
      int col2;
      if (Tcl_GetIntFromObj(interp, objv[0], &col2) != TCL_OK)
       return TCL_ERROR;
      if (Tcl_GetIntFromObj(interp, objv[1], &row2) != TCL_OK)
       return TCL_ERROR;
      if (row2 < row)
       {
        int t = row;
...
static int
Tix_GrSelModify(clientData, interp, argc, objv)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    else if (Tcl_GetString(objv[-1])[0] == 'c') {
	type = TIX_GR_CLEAR;
    }
    else if (Tcl_GetString(objv[-1])[0] == 's') {
	type = TIX_GR_SET;
    }
    else {
	type = TIX_GR_TOGGLE;
    ...
    if (Tcl_GetIntFromObj(interp, objv[0], &sbPtr->range[0][0]) != TCL_OK) {
	goto error;
    }
    if (Tcl_GetIntFromObj(interp, objv[1], &sbPtr->range[1][0]) != TCL_OK) {
	goto error;
    }
    if (argc == 4) {
	if (Tcl_GetIntFromObj(interp, objv[2], &sbPtr->range[0][1]) != TCL_OK) {
	    if (strcmp(Tcl_GetString(objv[2]), "max") == 0) {
	...
	if (Tcl_GetIntFromObj(interp, objv[3], &sbPtr->range[1][1]) != TCL_OK) {
	    if (strcmp(Tcl_GetString(objv[3]), "max") == 0) {
		Tcl_ResetResult(interp);
		sbPtr->range[1][1] = TIX_GR_MAX;
	    } else {
		goto error;
