...
static Tk_OptionSpec entryOptSpec[] = {
    ...
    {TK_OPTION_CALLBACK, "-validatecommand", "validateCommand", "ValidateCommand",
                NULL, -1, Tk_Offset(Entry, validateCmd),
       TK_OPTION_NULL_OK, 0, 0},
    ...
    Tcl_Obj *CONST objv[];      /* Argument objects. */
{
    ...
    Tcl_SetObjResult(interp,LangWidgetObj(interp,entryPtr->tkwin));
    return TCL_OK;
}
    ...
    Tcl_Obj *CONST objv[];      /* Argument objects. */
{
	...
	case COMMAND_VALIDATE: {
	    ...
	    code = EntryValidateChange(entryPtr,          NULL,
    ...
    int flags;                  /* Flags to pass to Tk_ConfigureWidget. */
{
	...
	if (entryPtr->type == TK_SPINBOX) {
		...
		Tcl_SetResult(interp, "-to value must be greater than -from value", TCL_VOLATILE);
    ...
    XEvent *eventPtr;           /* Information about event. */
{
	...
	case DestroyNotify:
		...
		Lang_DeleteWidget(entryPtr->interp, entryPtr->widgetCmd);
				 ...
				 * focusin or focusout */
{
	...
	Tcl_BackgroundError(entryPtr->interp);
	Tcl_SetResult(entryPtr->interp,          NULL, TCL_STATIC);
	code = TCL_ERROR;
	goto done;
    }
