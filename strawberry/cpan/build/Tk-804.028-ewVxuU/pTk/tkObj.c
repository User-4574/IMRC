    ...
    int *intPtr;                /* Place to store resulting pixels. */
{
    ...
    if (TclObjGetType(objPtr) != &pixelObjType) {
    ...
    register Tcl_Obj *copyObjPtr;       /* Object with internal rep to set. */
{
    ...
    TclObjSetType(copyObjPtr,TclObjGetType(srcObjPtr));
    ...
    double *doublePtr;          /* Place to store resulting millimeters. */
{
    ...
    if (TclObjGetType(objPtr) != &mmObjType) {
    ...
    mmPtr = (MMRep *) TclObjInternal(objPtr)->otherValuePtr;
...
{
    ckfree((char *) TclObjInternal(objPtr)->otherValuePtr);
    TclObjInternal(objPtr)->otherValuePtr = NULL;
    TclObjSetType(objPtr,NULL);
}
    ...
    register Tcl_Obj *copyObjPtr;       /* Object with internal rep to set. */
{
    ...
    TclObjSetType(copyObjPtr,TclObjGetType(srcObjPtr));
    oldPtr = (MMRep *) TclObjInternal(srcObjPtr)->otherValuePtr;
    newPtr = (MMRep *) ckalloc(sizeof(MMRep));
    newPtr->value = oldPtr->value;
    newPtr->units = oldPtr->units;
    newPtr->tkwin = oldPtr->tkwin;
    newPtr->returnValue = oldPtr->returnValue;
    TclObjInternal(copyObjPtr)->otherValuePtr = (VOID *) newPtr;
}
    ...
    Tcl_Obj *objPtr;            /* The object to convert. */
{
    ...
    if (TclObjGetType(objPtr) == tclDoubleObjType) {
	Tcl_GetDoubleFromObj(interp, objPtr, &d);
	units = -1;
    } else if (TclObjGetType(objPtr) == tclIntObjType) {
    ...
    TclObjSetType(objPtr,&mmObjType);
    ...
    TclObjInternal(objPtr)->otherValuePtr = (VOID *) mmPtr;
    ...
    Tk_Window *windowPtr;       /* Place to store resulting window. */
{
    ...
    winPtr = (WindowRep *) TclObjInternal(objPtr)->otherValuePtr;
    ...
    register Tcl_Obj *objPtr;   /* The object to convert. */
{
    ...
    TclObjInternal(objPtr)->otherValuePtr = (VOID*)winPtr;
    ...
    register Tcl_Obj *copyObjPtr;
{
    ...
    oldPtr = TclObjInternal(srcObjPtr)->otherValuePtr;
