    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
    ...
    if (count != 1) {
	Tcl_SetResult(interp, "Double or Triple modifier not allowed", TCL_STATIC);

	return TCL_ERROR;
    }
    if (*p != '\0') {
	Tcl_SetResult(interp, "only one event specification allowed", TCL_STATIC);
				 ...
				 * depends. */
{
	...
	if (eventMask & VirtualEventMask) {
	    if (allowVirtual == 0) {
		Tcl_SetResult(interp, "virtual event not allowed in definition of another virtual event", TCL_STATIC);
	...
	Tcl_SetResult(interp, "virtual events may not be composed", TCL_STATIC);
...
#define FIELD_SIZE 48
	...
	p = strchr(field, '>');
	if (p == field) {
	    Tcl_SetResult(interp, "virtual event \"<<>>\" is badly formed", TCL_STATIC);

	    count = 0;
	    goto done;
	}
	if ((p == NULL) || (p[1] != '>')) {
	    Tcl_SetResult(interp, "missing \">\" in virtual binding", TCL_STATIC);
	...
	*p = '>';
	...
	while (*p != '\0') {
	    p++;
	    if (*p == '>') {
		Tcl_SetResult(interp, "extra characters after detail in binding", TCL_STATIC);
