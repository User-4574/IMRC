...
static int		MwmProtocol _ANSI_ARGS_((Tcl_Interp * interp,
			    Tix_MwmInfo * wmPtr, int argc, Tcl_Obj *CONST *objv));
...
/* ARGSUSED */
int
Tix_MwmCmd(clientData, interp, argc, objv)
    ...
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		Tcl_GetString(objv[0]), " option pathname ?arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    c = Tcl_GetString(objv[1])[0];
    length = strlen(Tcl_GetString(objv[1]));

    if (!(winPtr = (TkWindow *) Tk_NameToWindow(interp, Tcl_GetString(objv[2]), tkwin))) {
	return TCL_ERROR;
    }
    if (!Tk_IsTopLevel(winPtr)) {
	Tcl_AppendResult(interp, Tcl_GetString(objv[2]), " is not a toplevel window.", NULL);
	return TCL_ERROR;
    }
    ...
    if ((c == 'd') && (strncmp(Tcl_GetString(objv[1]), "decorations", length) == 0)) {
	return SetMwmDecorations(interp, wmPtr, argc-3, objv+3);
    }
    else if ((c == 'i') && (strncmp(Tcl_GetString(objv[1]), "ismwmrunning", length) == 0)) {
    ...
    else if ((c == 'p') && (strncmp(Tcl_GetString(objv[1]), "protocol", length) == 0)) {
	return MwmProtocol(interp, wmPtr, argc-3, objv+3);
    }
    else if ((c == 't') && (strncmp(Tcl_GetString(objv[1]), "transientfor", length) == 0)) {
	return SetMwmTransientFor(interp, wmPtr, winPtr, argc-3, objv+3);
    }
    else {
	Tcl_AppendResult(interp, "unknown or ambiguous option \"",
	    Tcl_GetString(objv[1]), "\": must be decorations, ismwmrunning, protocol ",
	    "or transientfor",
	    NULL);
	return TCL_ERROR;
    }
 ...
 */
static
int SetMwmDecorations(interp, wmPtr, argc, objv)
    Tcl_Interp *interp;
    Tix_MwmInfo*wmPtr;
    int 	argc;
    Tcl_Obj *CONST *objv;
{
	...
	if (argc == 0) {
	     ...
	     * Query only one hint
	     */
	    if ((decor = MwmDecor(interp, Tcl_GetString(objv[0]))) == MWM_DECOR_UNKNOWN) {
		return TCL_ERROR;
	    }
	...
	for (i=0; i<argc; i+=2) {
	    ...
	    if ((decor = MwmDecor(interp, Tcl_GetString(objv[i]))) == MWM_DECOR_UNKNOWN)  {
		return TCL_ERROR;
	    }
...
static int MwmProtocol(interp, wmPtr, argc, objv)
    Tcl_Interp * interp;
    Tix_MwmInfo * wmPtr;
    int argc;
    Tcl_Obj *CONST *objv;
{
    ...
    if (strncmp(Tcl_GetString(objv[0]), "add", len) == 0 && argc == 3) {
	AddMwmProtocol(interp, wmPtr, Tcl_GetString(objv[1]), Tcl_GetString(objv[2]));
    }
    else if (strncmp(Tcl_GetString(objv[0]), "activate", len) == 0 && argc == 2) {
	ActivateMwmProtocol(interp, wmPtr, Tcl_GetString(objv[1]));
    }
    else if (strncmp(Tcl_GetString(objv[0]), "deactivate", len) == 0 && argc == 2) {
	DeactivateMwmProtocol(interp, wmPtr, Tcl_GetString(objv[1]));
    }
    else if (strncmp(Tcl_GetString(objv[0]), "delete", len) == 0 && argc == 2) {
	DeleteMwmProtocol(interp, wmPtr, Tcl_GetString(objv[1]));
    }
    else {
...
static
int SetMwmTransientFor(interp, wmPtr, mainWindow, argc, objv)
    Tcl_Interp *interp;
    Tix_MwmInfo*wmPtr;
    TkWindow   *mainWindow;
    int 	argc;
    Tcl_Obj *CONST *objv;
{
	...
	master = (TkWindow *) Tk_NameToWindow(interp, Tcl_GetString(objv[0]),
