...
#define STRING_LENGTH 400
    ...
    result = Tk_ConfigureWidget(interp, tkwin,
	    configSpecs, argc-2, objv+2, (char *) &psInfo,
	    TK_CONFIG_ARGV_ONLY);
    if (result != TCL_OK) {
	goto cleanup;
    }
					 ...
					 * is to be printed. */
{
	...
	list = Tcl_ObjGetVar2(interp, psInfoPtr->fontVar, name, 0);
        Tcl_DecrRefCount(name);
	if (list != NULL) {
	    if (Tcl_ListObjGetElements(interp, list, &argc, &objv)
		    != TCL_OK) {
		badMapEntry:
		Tcl_ResetResult(interp);
