 ...
 *----------------------------------------------------------------------
 */
int Tix_DItemConfigure(iPtr, argc, objv, flags)
    Tix_DItem * iPtr;
    int argc;
    Tcl_Obj *CONST *objv;
    int flags;
{
    return iPtr->base.diTypePtr->configureProc(iPtr, argc, objv, flags);
}
    ...
    Tix_ArgumentList *argListPtr;
{
	...
	ckfree((char*)argListPtr->arg[i].objv);
    }
 ...
 */
int
Tix_SplitConfig(interp, tkwin, specsList, numLists, argc, objv, argListPtr)
    Tcl_Interp * interp;
    Tk_Window tkwin;
    ...
    Tix_ArgumentList * argListPtr;
{
    ...
    for (n=0; n<argc; n+=2) {
	len = strlen(Tcl_GetString(objv[n]));
	found = 0;
	...
	for (i=0; i<numLists; i++) {
		...
		if (strncmp(Tcl_GetString(objv[n]), specPtr->argvName, len) == 0) {
	...
	if (found == 0) {
	    Tcl_AppendResult(interp, "unknown option \"", Tcl_GetString(objv[n]),
		    "\"",          NULL);
	    code = TCL_ERROR;
	    goto done;
	}
...
Tix_WidgetConfigure2(interp, tkwin, entRec, entConfigSpecs, iPtr,
		     argc, objv, flags, forced, sizeChanged_ret)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
    Tk_Window tkwin;		/* Window corresponding to widgRec. */
    char * entRec;
    Tk_ConfigSpec *entConfigSpecs; /* Describes legal options of the entry */
    ...
    int * sizeChanged_ret;
{
	...
	if (Tk_ConfigureWidget(interp, tkwin,
	    entConfigSpecs, argList.arg[0].argc, argList.arg[0].objv,
	...
	if (Tix_DItemConfigure(iPtr, argList.arg[1].argc,
	    argList.arg[1].objv, flags) != TCL_OK) {
	    code = TCL_ERROR;
	    goto done;
	}
