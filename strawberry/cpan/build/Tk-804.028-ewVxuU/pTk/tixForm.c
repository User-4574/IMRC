 ...
 */
int
Tix_FormCmd(clientData, interp, argc, objv)
 ...
 *----------------------------------------------------------------------
 */
static int TixFm_SetGrid(clientData, interp, argc, objv)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    master = Tk_NameToWindow(interp, Tcl_GetString(objv[0]), topLevel);
 ...
 *----------------------------------------------------------------------
 */
static int TixFm_Forget(clientData, interp, argc, objv)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    for (i=0; i<argc; i++) {
	clientPtr = TixFm_FindClientPtrByName(interp, Tcl_GetString(objv[i]), topLevel);
	if (clientPtr == NULL) {
	    return TCL_ERROR;
	}
	else {
 ...
 *----------------------------------------------------------------------
 */
static int TixFm_Slaves(clientData, interp, argc, objv)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if (masterPtr == 0) {
	Tcl_AppendResult(interp, "Window \"", Tcl_GetString(objv[0]),
 ...
 *----------------------------------------------------------------------
 */
static int TixFm_Spring(clientData, interp, argc, objv)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    ...
    if ((tkwin = Tk_NameToWindow(interp, Tcl_GetString(objv[0]), topLevel)) == NULL) {
	return TCL_ERROR;
    }
    ...
    if (Tcl_GetIntFromObj(interp, objv[2], &strength) != TCL_OK) {
	return TCL_ERROR;
    }

    len = strlen(Tcl_GetString(objv[1]));
    if (strncmp(Tcl_GetString(objv[1]), "-top", len) == 0) {
	i = 1; j = 0;
    }
    else if (strncmp(Tcl_GetString(objv[1]), "-bottom", len) == 0) {
	i = 1; j = 1;
    }
    else if (strncmp(Tcl_GetString(objv[1]), "-left", len) == 0) {
	i = 0; j = 0;
    }
    else if (strncmp(Tcl_GetString(objv[1]), "-right", len) == 0) {
	i = 0; j = 1;
    }
    else {
	Tcl_AppendResult(interp, "Unknown option \"", Tcl_GetString(objv[1]),
	    "\"", NULL);
	return TCL_ERROR;
    }
 ...
 *----------------------------------------------------------------------
 */
static int TixFm_Check(clientData, interp, argc, objv)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
 ...
 */
static int TixFm_CheckArgv(clientData, interp, argc, objv)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
    if ((argc >=1) && (Tcl_GetString(objv[0])[0] != '.')) {
...
static int TixFm_SetClient(clientData, interp, argc, objv)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Tcl_Obj *CONST *objv;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "can't put \"", pathName,
	    "\"in a form: it's a top-level window",          NULL);
	return TCL_ERROR;
    } else {
     ...
     */
    if (argc >= 2 && strcmp(Tcl_GetString(objv[0]), "-in")==0) {
	if ((master=Tk_NameToWindow(interp, Tcl_GetString(objv[1]), topLevel)) == NULL) {
	    return TCL_ERROR;
	}
	argc -= 2;
	objv += 2;
